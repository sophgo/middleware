...
#define DWA_MESH_EIS_POINT_CNT_MAX (256 * 256 * 4 * 2 * 2 * 4)
...
MESH_DATA_ALL_S g_MeshData[MESH_DATA_MAX_NUM];
MESH_DATA_EIS_S g_MeshEIS[MESH_DATA_MAX_NUM];
TSK_MESH_ATTR_S tskMesh[GDC_MAX_TSK_MESH];


static int dwa_init_eis(MESH_DATA_EIS_S *p_MeshEIS, int dst_width, int dst_height
	, int X_TILE_NUMBER, int Y_TILE_NUMBER, uint64_t mesh_tbl_phy_addr, int **reorder_mesh_tbl)
{
	...
	//int NUMBER_Y_LINE_A_TILE = (int)(ceil(ceil((float)dst_height / (float)Y_TILE_NUMBER) / (float)NUMBER_Y_LINE_A_SUBTILE)) * NUMBER_Y_LINE_A_SUBTILE;
	int MAX_MESH_NUM_A_TILE = ((64 * 2) / X_TILE_NUMBER) * (1 + ceil((4 * 128) / (float)dst_height));
	(void)dst_width;
	(void)Y_TILE_NUMBER;

	p_MeshEIS->nbr_mesh = 0;
	p_MeshEIS->buf_length = DWA_MESH_EIS_POINT_CNT_MAX * sizeof(int);

	p_MeshEIS->slice_tbl_lut = (int *)malloc(p_MeshEIS->buf_length);
	p_MeshEIS->slice_val_pos = (int *)malloc(p_MeshEIS->buf_length);
	p_MeshEIS->reorder_mesh_id_list = (int *)malloc(MAX_MESH_NUM_A_TILE * Y_SUBTILE_NUMBER * X_TILE_NUMBER * sizeof(int));
	//p_MeshEIS->reorder_mesh_tbl = (int *)malloc(X_TILE_NUMBER * Y_TILE_NUMBER);
	//p_MeshEIS->reorder_mesh_tbl[0] = (int *)malloc(p_MeshEIS->buf_length);

	if (!p_MeshEIS->slice_tbl_lut || !p_MeshEIS->slice_val_pos || !p_MeshEIS->reorder_mesh_id_list) {
		CVI_TRACE_GDC(CVI_DBG_ERR, "malloc fail.\n");
		goto malloc_fail;
	}

	p_MeshEIS->reorder_mesh_tbl[0] = reorder_mesh_tbl[0];
	p_MeshEIS->phy_addr = mesh_tbl_phy_addr;
	p_MeshEIS->vir_addr = (void *)reorder_mesh_tbl[0];
	p_MeshEIS->enable = true;

	CVI_TRACE_GDC(CVI_DBG_DEBUG, "p_MeshEIS enable\n");
	return 0;

malloc_fail:
	if (p_MeshEIS->slice_tbl_lut)
		free(p_MeshEIS->slice_tbl_lut);
	if (p_MeshEIS->slice_val_pos)
		free(p_MeshEIS->slice_val_pos);
	if (p_MeshEIS->reorder_mesh_id_list)
		free(p_MeshEIS->reorder_mesh_id_list);
...
int gdc_mesh_update_src_coordinate(int isrc_x_mesh_tbl[][4], int isrc_y_mesh_tbl[][4], MESH_DATA_EIS_S *p_MeshEIS)
{
	...
	int mesh_idx = 0;
	int cx = 0, cp = 0;
	int *reorder_mesh_slice_tbl = (int*)p_MeshEIS->vir_addr;
	//int *reorder_mesh_slice_tbl = p_MeshEIS->reorder_mesh_tbl[0];

	for (cx = 0; cx < p_MeshEIS->nbr_mesh; cx++) {
		mesh_idx = p_MeshEIS->slice_tbl_lut[cx];
		mesh = p_MeshEIS->slice_val_pos[cx];
		//cp = 0;
		for (int l = 0; l < 4; l++) {
			cp = (l << 2);
			reorder_mesh_slice_tbl[mesh_idx + cp] = isrc_x_mesh_tbl[mesh][l];
			reorder_mesh_slice_tbl[mesh_idx + cp+1] = isrc_y_mesh_tbl[mesh][l];
		}
	}
				 ...
				 MESH_DATA_EIS_S *p_MeshEIS)
{
	...
	for (int k = 0; k < Y_TILE_NUMBER; k++) {
		for (int j = 0; j < X_TILE_NUMBER; j++) {
			...
			if (p_MeshEIS->enable) {
				p_MeshEIS->slice_tbl_addr[j + k * X_TILE_NUMBER] = mesh_idx; // remember the address of each slice
				p_MeshEIS->slice_start_addr[j + k * X_TILE_NUMBER] = mesh_id_idx; // remember slice starting pos
			}
			...
			while (mesh_scan_tile_mesh_id_list[i] != MESH_ID_FSE) {
				...
				if (mesh == MESH_ID_FST || mesh == MESH_ID_FSS) {
					...
					if (reorder_id_map[mesh] == -1) {
						...
						// for eis
						if (p_MeshEIS->enable) {
							p_MeshEIS->slice_tbl_lut[cx] = mesh_idx;// mesh;
							p_MeshEIS->slice_val_pos[cx] = mesh;
							p_MeshEIS->nbr_mesh++;
							cx++;
						}
	...
	if (reorder_mesh_id_list[mesh_id_idx -1] != MESH_ID_FED) {
		CVI_TRACE_GDC(CVI_DBG_ERR, "invalid param, frame end id is not %#x\n", MESH_ID_FED);
	}
	...
	*/
	...
	for (k = 0xe0; k <= 0xff; k++) {
		//CVI_TRACE_GDC(CVI_DBG_DEBUG, "mesh_id_pos_lines:%#x, %#x, k:%#x\n" , mesh_id_pos_lines, mesh_id_pos_lines & 0xff, k);
		if ((mesh_id_pos_lines & 0xff) == k) {
			CVI_TRACE_GDC(CVI_DBG_DEBUG, "frm_end_tag in [%#x] pos, will put to next 2 lines(32byte)\n", mesh_id_pos_lines);
			for (int h = 0; h < 16; h++) {
...
void gdc_mesh_gen_affine(SIZE_S in_size, SIZE_S out_size, const AFFINE_ATTR_S *pstAffineAttr
...
static void dwa_node_to_mesh(FISHEYE_REGION_ATTR *FISHEYE_REGION, int rgn)
{
	CVI_TRACE_GDC(CVI_DBG_INFO, "node_to_mesh\n");
...
static int _get_region_all_mesh_data_memory(FISHEYE_REGION_ATTR *FISHEYE_REGION
	, int view_w, int view_h, int rgn, MESH_DATA_ALL_S *pmeshinfo)
{
	if (!FISHEYE_REGION[rgn].RegionValid) {
		CVI_TRACE_GDC(CVI_DBG_ERR, "region[%d] is not invalid\n", rgn);
		return -1;
	}
	int mesh_horcnt, mesh_vercnt;

	CVI_TRACE_GDC(CVI_DBG_INFO, "points to grids, hw mesh\n");
	...
	CVI_TRACE_GDC(CVI_DBG_INFO, "view_h = %d src,\n", view_h);
	CVI_TRACE_GDC(CVI_DBG_INFO, "view_w = %d,\n", view_w);
	CVI_TRACE_GDC(CVI_DBG_INFO, "mesh_horcnt = %d,\n", mesh_horcnt);
	CVI_TRACE_GDC(CVI_DBG_INFO, "mesh_vercnt = %d,\n", mesh_vercnt);
	...
	dwa_node_to_mesh(FISHEYE_REGION, rgn);

	CVI_TRACE_GDC(CVI_DBG_INFO, "PASS load mesh!!!!\n");

	return 0;
}


static int _load_region_config(FISHEYE_REGION_ATTR *FISHEYE_REGION, int rgn, MESH_DATA_ALL_S *meshd)
{
	if (meshd == NULL || !FISHEYE_REGION) {
		CVI_TRACE_GDC(CVI_DBG_ERR, "meshdata  or FISHEYE_REGION is NULL.\n");
		return -1;
	}
	...
	CVI_TRACE_GDC(CVI_DBG_INFO, "horcnt, vercnt, meshw, meshh, rx, ry: %d %d %d %d %d %d\n"
...
static int _get_region_mesh_list(FISHEYE_REGION_ATTR *FISHEYE_REGION, int rgn_idx, MESH_DATA_ALL_S *pMeshData)
{
	if (!FISHEYE_REGION) {
		CVI_TRACE_GDC(CVI_DBG_ERR, "invalid param, FISHEYE_REGION is NULL.\n");
		return -1;
	}
	if (!pMeshData) {
		CVI_TRACE_GDC(CVI_DBG_ERR, "invalid param, pMeshData is NULL.\n");
		return -2;
	}
	...
	if (pMeshData->balloc) {
		if (pMeshData->_nbr_mesh_x * pMeshData->_nbr_mesh_y > max_mesh_cnt) {
			CVI_TRACE_GDC(CVI_DBG_ERR, "mesh_x, mesh_y is too large.\n");
			return -1;
		}
	} else {
		CVI_TRACE_GDC(CVI_DBG_ERR, "g_MeshData is not alloc.\n");
		return -1;
	}

	// load output region from meshdata
	if (_load_region_config(FISHEYE_REGION, rgn_idx, pMeshData)) {
		CVI_TRACE_GDC(CVI_DBG_ERR, "load region cfg fail from meshdata.\n");
		return -1;
	}

	if (_get_region_all_mesh_data_memory(FISHEYE_REGION, tw, th, 0, pMeshData)) {
		CVI_TRACE_GDC(CVI_DBG_ERR, "get region meshdata fail.\n");
		return -1;
	}
 ...
 */
static int generate_mesh_on_fisheye(const GRID_INFO_ATTR_S* pstGridInfoAttr, FISHEYE_ATTR* FISHEYE_CONFIG, FISHEYE_REGION_ATTR* FISHEYE_REGION
	...
	, uint16_t *reorder_mesh_id_list, int **reorder_mesh_tbl, uint64_t mesh_tbl_phy_addr)
{
	...
	for (int rgn_idx = 0; rgn_idx < FISHEYE_CONFIG->RgnNum; rgn_idx++) {
		...
		if (!FISHEYE_REGION[rgn_idx].RegionValid) {
			CVI_TRACE_GDC(CVI_DBG_ERR, "Region invalid.\n");
			return -1;
		}
		if (FISHEYE_REGION[rgn_idx].ViewMode == PROJECTION_STEREO_FIT) {
			if (!pstGridInfoAttr) {
				CVI_TRACE_GDC(CVI_DBG_ERR, "pstGridInfoAttr null\n");
				return -1;
			}
			...
			if (g_MeshData[grid_idx].slice_info.magic == SLICE_MAGIC) {
				X_TILE_NUMBER = g_MeshData[grid_idx].slice_info.slice_h_cnt;
				Y_TILE_NUMBER = g_MeshData[grid_idx].slice_info.slice_v_cnt;
			}
			if (_get_region_mesh_list(FISHEYE_REGION, rgn_idx, &g_MeshData[grid_idx])) {
				CVI_TRACE_GDC(CVI_DBG_ERR, "invalid param, FISHEYE_REGION is NULL.\n");
				return -1;
			}
			if (pstGridInfoAttr->bEISEnable) {
				if (dwa_init_eis(&g_MeshEIS[grid_idx], g_MeshData[grid_idx].imgw, g_MeshData[grid_idx].imgh
					, X_TILE_NUMBER, Y_TILE_NUMBER, mesh_tbl_phy_addr, reorder_mesh_tbl)) {
					CVI_TRACE_GDC(CVI_DBG_ERR, "dwa_init_eis err\n");
					return -1;
				}
			}
		} else {
			...
			grid_idx = 0;
			g_MeshData[grid_idx].grid_mode = GRID_MODE_REGION_BASE;
			g_MeshEIS[grid_idx].enable = false;
...
CVI_S32 gdc_mesh_gen_fisheye(SIZE_S in_size, SIZE_S out_size, const FISHEYE_ATTR_S *pstFishEyeAttr
	, uint64_t mesh_phy_addr, void *mesh_vir_addr, ROTATION_E rot)
{
	...
	CVI_TRACE_GDC(CVI_DBG_DEBUG, "mesh_vir_addr:%p, reorder_mesh_tbl[0]:%p\n", mesh_vir_addr, reorder_mesh_tbl[0]);
...
CVI_S32 gdc_mesh_gen_warp(SIZE_S in_size, SIZE_S out_size, const WARP_ATTR_S *pstWarpAttr
	, uint64_t mesh_phy_addr, void *mesh_vir_addr)
{
	FISHEYE_ATTR *FISHEYE_CONFIG;
	FISHEYE_REGION_ATTR *FISHEYE_REGION;
			...
			uint64_t mesh_phy_addr, void *mesh_vir_addr)
{
	...
	FISHEYE_CONFIG->rotate_index = pstLDCAttr->enRotation;
	...
	ret = generate_mesh_on_fisheye(&(pstLDCAttr->stGridInfoAttr), FISHEYE_CONFIG, FISHEYE_REGION, X_TILE_NUMBER, Y_TILE_NUMBER
		...
		, reorder_mesh_tbl, mesh_tbl_phy_addr);
	if (ret) {
		CVI_TRACE_GDC(CVI_DBG_ERR, "generate_mesh_on_fisheye fail\n");
	}
...
int cnv_mesh_tbl_reorder_and_parse_3(uint16_t *mesh_scan_tile_mesh_id_list, int mesh_id_list_entry_num,
				     ...
				     uint64_t mesh_tbl_phy_addr, int imgw, int imgh)
{
	int mesh = -1;
	int reorder_mesh;
	int mesh_id_idx = 0;
...
{
	CVI_U8 i = GDC_MAX_TSK_MESH;

	if (!name) {
		CVI_TRACE_GDC(CVI_DBG_ERR, "tsk mesh name is null\n");
		return i;
	}

	for (i = 0; i < GDC_MAX_TSK_MESH; i++) {
		if (strcmp(tskMesh[i].Name, name) == 0 && tskMesh[i].paddr && tskMesh[i].vaddr) {
			CVI_TRACE_GDC(CVI_DBG_DEBUG, "got remain tsk mesh[%d-%s-%#"PRIx64"]\n", i, tskMesh[i].Name, tskMesh[i].paddr);
			break;
		}
	}

	if (i == GDC_MAX_TSK_MESH)
		CVI_TRACE_GDC(CVI_DBG_DEBUG, "start alloc new tsk mesh[%s]\n", name);
	return i;
}
...
void free_cur_tsk_mesh(CVI_CHAR *meshName)
{
	...
	i = get_valid_tsk_mesh_by_name2(meshName);
	if (i < GDC_MAX_TSK_MESH && tskMesh[i].paddr && tskMesh[i].vaddr) {
		CVI_TRACE_GDC(CVI_DBG_DEBUG, "start free tsk mesh[%s]\n", meshName);
		CVI_SYS_IonFree(tskMesh[i].paddr, tskMesh[i].vaddr);
		tskMesh[i].paddr = 0;
		tskMesh[i].vaddr = 0;
		...
		const char *name, CVI_U64 *pu64PhyAddr, CVI_VOID **ppVirAddr)
{
	...
	if (idx >= GDC_MAX_TSK_MESH) {
		...
		if (gdc_mesh_gen_ldc(in_size, out_size, pstLDCAttr, paddr, vaddr) != CVI_SUCCESS) {
	...
	*pu64PhyAddr = tskMesh[idx].paddr;
	*ppVirAddr = tskMesh[idx].vaddr;

	CVI_TRACE_GDC(CVI_DBG_DEBUG, "tsk mesh(%#"PRIx64")\n", *pu64PhyAddr);
...
{
	free_cur_tsk_mesh(tskName);
	free_cur_meshdata(tskName);
}
		...
		const char *name, CVI_U64 *pu64PhyAddr, CVI_VOID **ppVirAddr)
{
	...
	if (pstFishEyeAttr->stGridInfoAttr.Enable) {
		if (pstFishEyeAttr->stGridInfoAttr.grid_in.u32Width && pstFishEyeAttr->stGridInfoAttr.grid_in.u32Height) {
			in_size.u32Width = pstFishEyeAttr->stGridInfoAttr.grid_in.u32Width;
			in_size.u32Height = pstFishEyeAttr->stGridInfoAttr.grid_in.u32Height;
		}
		if (pstFishEyeAttr->stGridInfoAttr.grid_out.u32Width &&
			pstFishEyeAttr->stGridInfoAttr.grid_out.u32Height) {
			out_size.u32Width = pstFishEyeAttr->stGridInfoAttr.grid_out.u32Width;
			out_size.u32Height = pstFishEyeAttr->stGridInfoAttr.grid_out.u32Height;
		}
	}
	...
	if (idx >= GDC_MAX_TSK_MESH) {
		...
		// acquire memory space for mesh.
		if (CVI_SYS_IonAlloc_Cached(&paddr, &vaddr, name, CVI_GDC_MESH_SIZE_FISHEYE) != CVI_SUCCESS) {
			CVI_TRACE_GDC(CVI_DBG_ERR, "Can't acquire memory for mesh.\n");
			return CVI_ERR_GDC_NOMEM;
		}

		if (gdc_mesh_gen_fisheye(in_size, out_size, pstFishEyeAttr, paddr, vaddr, enRotation) != CVI_SUCCESS) {
			CVI_SYS_IonFree(paddr, vaddr);
			CVI_TRACE_GDC(CVI_DBG_ERR, "Can't generate ldc mesh.\n");
			return CVI_ERR_GDC_ILLEGAL_PARAM;
		}
