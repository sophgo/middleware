...
static CVI_S32 _get_1st_src_midxy(int num_tilex_s1,
	LDC_ATTR *cfg, COORD2D_INT_HW *src_1st_list, int tidx, int tidy, int mesh_horcnt, int mesh_vercnt, const LDC_ATTR_S *pstLDCAttr)
{
...
static CVI_S32 _offline_get_1st_src_mesh_table(int num_tiley_s1,
					       int num_tilex_s1,
					       LDC_ATTR *cfg,
					       COORD2D_INT_HW *src_1st_list, int mesh_horcnt, int mesh_vercnt, const LDC_ATTR_S *pstLDCAttr)
{
	CVI_S32 ret;
...
static void gdc_get_region_src_mesh_list(LDC_RGN_ATTR *rgn_attr, int rgn_idx,
	...
	void *mesh_vir_addr, ROTATION_E rot)
{
	...
	for (int rgn_idx = 0; rgn_idx < cfg->RgnNum; rgn_idx++) {
		...
		clock_gettime(CLOCK_MONOTONIC, &start);
#endif
		gdc_get_region_src_mesh_list(rgn_attr, rgn_idx, x0, y0, &gdc_meshdata, pstLDCAttr); //, mat0);
#ifdef ENABLE_PROFILE
		clock_gettime(CLOCK_MONOTONIC, &end);
...
static CVI_U8 gdc_get_valid_tsk_mesh_by_name(const CVI_CHAR *name)
{
...
static CVI_U8 gdc_get_valid_tsk_mesh_by_name2(const CVI_CHAR *name)
{
	CVI_U8 i = GDC_MAX_TSK_MESH;
...
static CVI_U8 gdc_get_idle_tsk_mesh(void)
{
	CVI_U8 i = GDC_MAX_TSK_MESH;
...
static void gdc_free_cur_tsk_mesh(CVI_CHAR *meshName)
{
	CVI_U8 i = GDC_MAX_TSK_MESH;

	i = gdc_get_valid_tsk_mesh_by_name2(meshName);
		...
		const char *name, CVI_U64 *pu64PhyAddr, CVI_VOID **ppVirAddr)
{
	...
	CVI_U8 idx = gdc_get_valid_tsk_mesh_by_name(name);
	...
	if (idx >= GDC_MAX_TSK_MESH) {
		...
		if (fwrite((void *)ALIGN((CVI_U64)vaddr, DEFAULT_ALIGN), mesh_size, 1, fp) != 1)
			CVI_TRACE_GDC(CVI_DBG_ERR, "fwrite data error\n");
		fclose(fp);
dump_fail:
#endif
	...
	, const char *tskName, CVI_U64 *pu64PhyAddr, CVI_VOID **ppVirAddr)
{
	...
	CVI_U8 idx = gdc_get_valid_tsk_mesh_by_name(tskName);
	if (idx >= GDC_MAX_TSK_MESH) {
		mesh_gen_get_size(in_size, out_size, &mesh_1st_size, &mesh_2nd_size);
		mesh_size = mesh_1st_size + mesh_2nd_size;
