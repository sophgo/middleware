...
int load_meshdata(const char *path, MESH_DATA_ALL_S *pmeshdata,
	const char *bindName, void *pBuf, int Len)
{
	int info[100] = {0};
	FILE *fpGrid = NULL;
	int *buffer = pBuf;
	...
	if (pBuf && Len) {
		...
		memcpy(info, buffer, sizeof(int) * 100);
		buffer += 100;
	} else {
	...
	if (count_grid > 0) {
		if (pBuf && Len) {
			...
			memcpy(pmeshdata->pgrid_src, buffer, sizeof(int) * (count_grid * 2));
			buffer += (count_grid * 2);
			...
			memcpy(pmeshdata->pgrid_dst, buffer, sizeof(int) * (count_grid * 2));
			buffer += (count_grid * 2);
	...
	if (pmeshdata->node_index > 0) {
		if (pBuf && Len) {
			// hw node
			length += (pmeshdata->node_index * 2);
			if (length > Len) {
				CVI_TRACE_GDC(CVI_DBG_ERR, "load gridinfo fail\n");
				return -1;
			}
			memcpy(pmeshdata->pnode_src, buffer, sizeof(int) * (pmeshdata->node_index * 2));
			buffer += (pmeshdata->node_index * 2);
			...
			memcpy(pmeshdata->pnode_dst, buffer, sizeof(int) * (pmeshdata->node_index * 2));
			buffer += (pmeshdata->node_index * 2);
		} else {
			// hw node
