...
TSK_MESH_ATTR_S dwa_tskMesh[DWA_MAX_TSK_MESH];
	...
	, int NUM_Y_LINE_A_SUBTILE, int Y_TILE_NUMBER, int MAX_MESH_NUM_A_TILE)
{
	...
	CVI_TRACE_DWA(CVI_DBG_DEBUG, "X_TILE_NUMBER(%d) Y_TILE_NUMBER(%d)\n", X_TILE_NUMBER, Y_TILE_NUMBER);
	CVI_TRACE_DWA(CVI_DBG_DEBUG, "NUM_X_LINE_A_TILE(%d) NUM_Y_LINE_A_TILE(%d)\n"
		, NUM_X_LINE_A_TILE, NUM_Y_LINE_A_TILE);
	CVI_TRACE_DWA(CVI_DBG_DEBUG, "NUM_Y_LINE_A_SUBTILE(%d)\n", NUM_Y_LINE_A_SUBTILE);
				 ...
				 int *reorder_mesh_id_list_entry_num, uint64_t mesh_tbl_phy_addr, int imgw, int imgh)
{
	...
	if (reorder_mesh_id_list[mesh_id_idx -1] != MESH_ID_FED) {
		CVI_TRACE_DWA(CVI_DBG_ERR, "invalid param, frame end id is not %#x\n", MESH_ID_FED);
	}
	...
	*/
	...
	for (k = 0xe0; k <= 0xff; k++) {
		...
		if ((mesh_id_pos_lines & 0xff) == k) {
			CVI_TRACE_DWA(CVI_DBG_DEBUG, "frm_end_tag in [%#x] pos, will put to next 2 lines(32byte)\n", mesh_id_pos_lines);
			for (int h = 0; h < 16; h++) {
...
void dwa_mesh_gen_affine(SIZE_S in_size, SIZE_S out_size, const AFFINE_ATTR_S *pstAffineAttr
	, uint64_t mesh_phy_addr, void *mesh_vir_addr)
{
	...
	CVI_TRACE_DWA(CVI_DBG_DEBUG, "phy-addr of mesh id(%#"PRIx64") mesh_tbl(%#"PRIx64")\n"
		     , mesh_id_phy_addr, mesh_tbl_phy_addr);
	CVI_TRACE_DWA(CVI_DBG_DEBUG, "mesh_id_size(%d) mesh_tbl_size(%d)\n", mesh_id_size, mesh_tbl_size);
...
void _Panorama180View2(FISHEYE_REGION_ATTR* FISHEYE_REGION, int rgn_idx, FISHEYE_MOUNT_MODE_E MOUNT, double x0, double y0, double r)
{
	...
	if (MOUNT != FISHEYE_WALL_MOUNT) {
		CVI_TRACE_DWA(CVI_DBG_ERR, "mount_mode(%d) not supported in Panorama180\n", MOUNT);
		return;
	}
...
void _Panorama360View2( FISHEYE_REGION_ATTR* FISHEYE_REGION, int rgn_idx, FISHEYE_MOUNT_MODE_E MOUNT, double x0, double y0, double r)
{
	...
	if ((MOUNT != FISHEYE_CEILING_MOUNT) && (MOUNT != FISHEYE_DESKTOP_MOUNT)) {
		CVI_TRACE_DWA(CVI_DBG_ERR, "mount_mode(%d) not supported in Panorama360\n", MOUNT);
		return;
	}
...
{
	CVI_TRACE_DWA(CVI_DBG_INFO, "node_to_mesh\n");
	...
	, int view_w, int view_h, int rgn, MESH_DATA_ALL_S *pmeshinfo)
{
	if (!FISHEYE_REGION[rgn].RegionValid) {
		CVI_TRACE_DWA(CVI_DBG_ERR, "region[%d] is not invalid\n", rgn);
		return -1;
	}
	int mesh_horcnt, mesh_vercnt;

	CVI_TRACE_DWA(CVI_DBG_INFO, "points to grids, hw mesh\n");
	...
	CVI_TRACE_DWA(CVI_DBG_INFO, "view_h = %d src,\n", view_h);
	CVI_TRACE_DWA(CVI_DBG_INFO, "view_w = %d,\n", view_w);
	CVI_TRACE_DWA(CVI_DBG_INFO, "mesh_horcnt = %d,\n", mesh_horcnt);
	CVI_TRACE_DWA(CVI_DBG_INFO, "mesh_vercnt = %d,\n", mesh_vercnt);
	...
	CVI_TRACE_DWA(CVI_DBG_INFO, "PASS load mesh!!!!\n\r");
...
static int _load_region_config(FISHEYE_REGION_ATTR *FISHEYE_REGION, int rgn, MESH_DATA_ALL_S *meshd)
{
	if (meshd == NULL || !FISHEYE_REGION) {
		CVI_TRACE_DWA(CVI_DBG_ERR, "meshdata  or FISHEYE_REGION is NULL.\n");
		return -1;
	}
	...
	CVI_TRACE_DWA(CVI_DBG_INFO, "horcnt, vercnt, meshw, meshh, rx, ry: %d %d %d %d %d %d\n"
...
{
	if (!FISHEYE_REGION) {
		CVI_TRACE_DWA(CVI_DBG_ERR, "invalid param, FISHEYE_REGION is NULL.\n");
		return -1;
	}
	if (!pMeshData) {
		CVI_TRACE_DWA(CVI_DBG_ERR, "invalid param, pMeshData is NULL.\n");
		return -2;
	}
	...
	if (pMeshData->balloc) {
		if (pMeshData->_nbr_mesh_x * pMeshData->_nbr_mesh_y > max_mesh_cnt) {
			CVI_TRACE_DWA(CVI_DBG_ERR, "mesh_x, mesh_y is too large.\n");
			return -1;
		}
	} else {
		CVI_TRACE_DWA(CVI_DBG_ERR, "g_MeshData is not alloc.\n");
		return -1;
	}

	// load output region from meshdata
	if (_load_region_config(FISHEYE_REGION, rgn_idx, pMeshData)) {
		CVI_TRACE_DWA(CVI_DBG_ERR, "load region cfg fail from meshdata.\n");
		return -1;
	}

	if (_get_region_all_mesh_data_memory(FISHEYE_REGION, tw, th, 0, pMeshData)) {
		CVI_TRACE_DWA(CVI_DBG_ERR, "get region meshdata fail.\n");
		return -1;
	}
...
void _get_region_dst_mesh_list(FISHEYE_REGION_ATTR* FISHEYE_REGION, int rgn_idx)
{
	...
	CVI_TRACE_DWA(CVI_DBG_DEBUG, "knotcnt(%d)\n", knotcnt);
	// tmp for debug
	...
	, uint16_t *reorder_mesh_id_list, int **reorder_mesh_tbl, uint64_t mesh_tbl_phy_addr)
{
	...
	for (int rgn_idx = 0; rgn_idx < FISHEYE_CONFIG->RgnNum; rgn_idx++) {
		...
		if (!FISHEYE_REGION[rgn_idx].RegionValid) {
			CVI_TRACE_DWA(CVI_DBG_ERR, "Region invalid.\n");
			return -1;
		}
		if (FISHEYE_REGION[rgn_idx].ViewMode == PROJECTION_STEREO_FIT) {
			...
			if (load_meshdata(pstGridInfoAttr->gridFileName, &g_MeshData[grid_idx], pstGridInfoAttr->gridBindName)) {
				CVI_TRACE_DWA(CVI_DBG_ERR, "load_meshdata fail.\n");
				return -1;
			}
	...
	CVI_TRACE_DWA(CVI_DBG_INFO, "mesh_tbl_num = %d\n", mesh_tbl_num);
	...
	CVI_TRACE_DWA(CVI_DBG_DEBUG, "mesh_id_list_entry_num = %d\n", mesh_id_list_entry_num);
	...
	for (int i = 0; i < mesh_tbl_num; i++) {
		for (int j = 0; j < 4; j++) {
			...
			fwrite(&idst_y_mesh_tbl[i][j], sizeof(int), 1, fpIntmesh);
		}
	}
	fclose(fpIntmesh);
#endif
	...
	fpDbg = fopen("reorder_mesh_tbl.bin", "wb");
	fwrite(reorder_mesh_tbl[0], sizeof(int), reorder_mesh_tbl_entry_num, fpDbg);
	fclose(fpDbg);
	fpDbg = fopen("reorder_mesh_id_list.bin", "wb");
	fwrite(reorder_mesh_id_list, sizeof(int), reorder_mesh_id_list_entry_num, fpDbg);
	fclose(fpDbg);
#endif

	CVI_TRACE_DWA(CVI_DBG_INFO, "mesh table size (bytes) = %d\n", (reorder_mesh_tbl_entry_num * 4));
	CVI_TRACE_DWA(CVI_DBG_INFO, "mesh id list size (bytes) = %d\n", (reorder_mesh_id_list_entry_num * 2));

	clock_gettime(CLOCK_MONOTONIC, &end);
	CVI_TRACE_DWA(CVI_DBG_INFO, "time consumed: %fms\n",
	...
	, FISHEYE_REGION_ATTR *FISHEYE_REGION)
{
	...
	CVI_TRACE_DWA(CVI_DBG_INFO, "OutW_disp(%d) OutH_disp(%d)\n"
		, FISHEYE_CONFIG->OutW_disp, FISHEYE_CONFIG->OutH_disp);
	CVI_TRACE_DWA(CVI_DBG_INFO, "InCenterX(%d) InCenterY(%d)\n"
		, FISHEYE_CONFIG->InCenterX, FISHEYE_CONFIG->InCenterY);
	CVI_TRACE_DWA(CVI_DBG_INFO, "FStrength(%lf) TCoef(%lf) RgnNum(%d)\n"
	...
	for (int i = 0; i < FISHEYE_CONFIG->RgnNum; ++i) {
		...
		CVI_TRACE_DWA(CVI_DBG_INFO, "Region(%d) ViewMode(%d) MeshVer(%d) MeshHor(%d)\n"
			, i, FISHEYE_REGION[i].ViewMode, FISHEYE_REGION[i].MeshVer, FISHEYE_REGION[i].MeshHor);
		CVI_TRACE_DWA(CVI_DBG_INFO, "ZoomH(%lf) ZoomV(%lf) Pan(%d) Tilt(%d) PanEnd(%d)\n"
			, FISHEYE_REGION[i].ZoomH, FISHEYE_REGION[i].ZoomV
			, FISHEYE_REGION[i].Pan, FISHEYE_REGION[i].Tilt, FISHEYE_REGION[i].PanEnd);
		CVI_TRACE_DWA(CVI_DBG_INFO, "InRadius(%lf) OutRadius(%lf) Rect(%d %d %d %d)\n"
...
CVI_S32 dwa_mesh_gen_fisheye(SIZE_S in_size, SIZE_S out_size, const FISHEYE_ATTR_S *pstFishEyeAttr
	...
	, FISHEYE_REGION_ATTR *FISHEYE_REGION)
{
	...
	for (int i = 0; i < FISHEYE_CONFIG->RgnNum; ++i) {
		...
		CVI_TRACE_DWA(CVI_DBG_INFO, "Region(%d) ViewMode(%d) MeshVer(%d) MeshHor(%d)\n"
			, i, FISHEYE_REGION[i].ViewMode, FISHEYE_REGION[i].MeshVer, FISHEYE_REGION[i].MeshHor);
		CVI_TRACE_DWA(CVI_DBG_INFO, "bAspect(%d) XYRatio(%d) DistortionRatio(%d)\n"
			, (bool)FISHEYE_REGION[i].ZoomV, FISHEYE_REGION[i].Pan, FISHEYE_REGION[i].PanEnd);
		CVI_TRACE_DWA(CVI_DBG_INFO, "XRatio(%d) XYRatio(%d)\n"
			, (int)FISHEYE_REGION[i].ThetaX, (int)FISHEYE_REGION[i].ThetaZ);
		CVI_TRACE_DWA(CVI_DBG_INFO, "CenterXOffset(%lf) CenterYOffset(%lf) Rect(%d %d %d %d)\n"
			, FISHEYE_REGION[i].InRadius, FISHEYE_REGION[i].OutRadius
			, FISHEYE_REGION[i].OutX, FISHEYE_REGION[i].OutY
			, FISHEYE_REGION[i].OutW, FISHEYE_REGION[i].OutH);
	}
				  ...
				  int **reorder_mesh_tbl, uint64_t mesh_tbl_phy_addr)
{
	...
	cnv_mesh_tbl_reorder_and_parse_3(mesh_scan_tile_mesh_id_list, mesh_id_list_entry_num, isrc_x_mesh_tbl,
					 ...
					 reorder_mesh_id_list, &reorder_mesh_id_list_entry_num, mesh_tbl_phy_addr, dst_width, dst_height);
...
CVI_U8 get_valid_tsk_mesh_by_name(const CVI_CHAR *name)
{
	...
	if (!name) {
		CVI_TRACE_DWA(CVI_DBG_ERR, "tsk mesh name is null\n");
		return i;
	}

	for (i = 0; i < DWA_MAX_TSK_MESH; i++) {
		if (strcmp(dwa_tskMesh[i].Name, name) == 0 && dwa_tskMesh[i].paddr && dwa_tskMesh[i].vaddr) {
			CVI_TRACE_DWA(CVI_DBG_DEBUG, "got remain tsk mesh[%d-%s-%#"PRIx64"]\n", i, dwa_tskMesh[i].Name, dwa_tskMesh[i].paddr);
			break;
		}
	}

	if (i == DWA_MAX_TSK_MESH)
		CVI_TRACE_DWA(CVI_DBG_DEBUG, "start alloc new tsk mesh[%s]\n", name);
	return i;
}
...
void free_cur_tsk_mesh(CVI_CHAR *meshName)
{
	...
	if (i < DWA_MAX_TSK_MESH && dwa_tskMesh[i].paddr && dwa_tskMesh[i].vaddr) {
		CVI_TRACE_DWA(CVI_DBG_DEBUG, "start free tsk mesh[%s]\n", meshName);
		CVI_SYS_IonFree(dwa_tskMesh[i].paddr, dwa_tskMesh[i].vaddr);
		dwa_tskMesh[i].paddr = 0;
		dwa_tskMesh[i].vaddr = 0;
		memset(dwa_tskMesh[i].Name, 0, sizeof(dwa_tskMesh[i].Name));
	}
}
...
void free_all_tsk_mesh(void)
{
	...
	for (i = 0; i < DWA_MAX_TSK_MESH; i++) {
		if (dwa_tskMesh[i].paddr && dwa_tskMesh[i].vaddr) {
			CVI_TRACE_DWA(CVI_DBG_DEBUG, "start free tsk mesh[%s]\n", dwa_tskMesh[i].Name);
			CVI_SYS_IonFree(dwa_tskMesh[i].paddr, dwa_tskMesh[i].vaddr);
			dwa_tskMesh[i].paddr = 0;
			dwa_tskMesh[i].vaddr = 0;
		...
		const char *name, CVI_U64 *pu64PhyAddr, CVI_VOID **ppVirAddr)
{
	...
	if (idx >= DWA_MAX_TSK_MESH) {
		...
		if (dwa_mesh_gen_ldc(in_size, out_size, pstLDCAttr, paddr, vaddr) != CVI_SUCCESS) {
			CVI_SYS_IonFree(paddr, vaddr);
			CVI_TRACE_DWA(CVI_DBG_ERR, "Can't generate ldc mesh.\n");
			return CVI_ERR_DWA_ILLEGAL_PARAM;
		}
		...
		if (!fp) {
			CVI_TRACE_DWA(CVI_DBG_ERR, "open file[%s] failed.\n", mesh_name);
			goto dump_fail;
		}
		if (fwrite((void *)ALIGN((CVI_U64)vaddr, DEFAULT_ALIGN), mesh_size, 1, fp) != 1)
			CVI_TRACE_DWA(CVI_DBG_ERR, "fwrite data error\n");
		fclose(fp);
dump_fail:
#endif
	...
	*pu64PhyAddr = dwa_tskMesh[idx].paddr;
	*ppVirAddr = dwa_tskMesh[idx].vaddr;

	CVI_TRACE_DWA(CVI_DBG_DEBUG, "tsk mesh(%#"PRIx64")\n", *pu64PhyAddr);
		...
		const char *name, CVI_U64 *pu64PhyAddr, CVI_VOID **ppVirAddr)
{
	...
	if (idx >= DWA_MAX_TSK_MESH) {
		...
		// acquire memory space for mesh.
		if (CVI_SYS_IonAlloc_Cached(&paddr, &vaddr, name, CVI_DWA_MESH_SIZE_FISHEYE) != CVI_SUCCESS) {
			CVI_TRACE_DWA(CVI_DBG_ERR, "Can't acquire memory for mesh.\n");
			return CVI_ERR_DWA_NOMEM;
		}

		if (dwa_mesh_gen_fisheye(in_size, out_size, pstFishEyeAttr, paddr, vaddr, enRotation) != CVI_SUCCESS) {
			CVI_SYS_IonFree(paddr, vaddr);
			CVI_TRACE_DWA(CVI_DBG_ERR, "Can't generate ldc mesh.\n");
			return CVI_ERR_DWA_ILLEGAL_PARAM;
		}
