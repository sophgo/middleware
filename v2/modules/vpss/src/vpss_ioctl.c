...
{
	return ioctl(fd, CVI_VPSS_RELEASE_CHN_RAME, cfg);
}
...
CVI_S32 vpss_set_chn_fisheye(CVI_S32 fd, const struct vpss_chn_fisheye_cfg *cfg)
{
	return ioctl(fd, CVI_VPSS_SET_CHN_FISHEYE, cfg);
}

CVI_S32 vpss_get_chn_fisheye(CVI_S32 fd, struct vpss_chn_fisheye_cfg *cfg)
{
	return ioctl(fd, CVI_VPSS_GET_CHN_FISHEYE, cfg);
}
...
CVI_S32 vpss_set_draw_rect(CVI_S32 fd, const struct vpss_chn_draw_rect_cfg *cfg)
{
	return ioctl(fd, CVI_VPSS_SET_CHN_DRAW_RECT, cfg);
}

CVI_S32 vpss_get_draw_rect(CVI_S32 fd, struct vpss_chn_draw_rect_cfg *cfg)
{
	return ioctl(fd, CVI_VPSS_GET_CHN_DRAW_RECT, cfg);
}

CVI_S32 vpss_set_convert(CVI_S32 fd, const struct vpss_chn_convert_cfg *cfg)
{
	return ioctl(fd, CVI_VPSS_SET_CHN_CONVERT, cfg);
}

CVI_S32 vpss_get_convert(CVI_S32 fd, struct vpss_chn_convert_cfg *cfg)
{
	return ioctl(fd, CVI_VPSS_GET_CHN_CONVERT, cfg);
}
...
CVI_S32 vpss_set_mod_param(CVI_S32 fd, const VPSS_MOD_PARAM_S *cfg)
{
	return ioctl(fd, CVI_VPSS_SET_MOD_PARAM, cfg);
}

CVI_S32 vpss_get_mod_param(CVI_S32 fd, VPSS_MOD_PARAM_S *cfg)
{
	return ioctl(fd, CVI_VPSS_GET_MOD_PARAM, cfg);
}
...
CVI_S32 vpss_stitch(CVI_S32 fd, struct _vpss_stitch_cfg *cfg)
{
	return ioctl(fd, CVI_VPSS_STITCH, cfg);
}
