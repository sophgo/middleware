 ...
 *
 * File Name: include/g726.c
 * Description:
 *   Common audio  definitions & api for application.
 */
...
static short fmult(short an, short srn)
{
	...
	anmant = (anmag == 0)  ?  32  : (anexp >= 0)  ? (anmag >> anexp)  :
		 (anmag << -anexp);
...
static inline short predictor_zero(g726_state_t *s)
{
	int i;
	int sezi;
...
static inline short predictor_pole(g726_state_t *s)
{
...
g726_state_t *g726_init(g726_state_t *s, int bit_rate)
{
	...
	if (bit_rate != 16000  &&  bit_rate != 24000  &&  bit_rate != 32000
	    &&  bit_rate != 40000)
		return NULL;
		...
		int g726_bytes)
{
	...
	//printf("[v]g726 decode input[%d]bytes return[%d]samples\n", g726_bytes ,samples);
	return samples;
}
		...
		int len)
{
	...
	for (g726_bytes = i = 0;  i < len;  i++) {
		...
		s->bs.residue += s->bits_per_sample;
		if (s->bs.residue >= 8) {
			g726_data[g726_bytes++] = (unsigned char)((s->bs.bitstream >>
						  (s->bs.residue - 8)) & 0xFF);
			s->bs.residue -= 8;
		}

	}
	//printf("[v]g726 encode input[%d]samples return[%d]bytes\n", len ,g726_bytes);
	return g726_bytes;
}
