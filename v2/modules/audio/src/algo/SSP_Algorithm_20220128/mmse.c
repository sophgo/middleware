...
EXPORT void NR(short *x, float initial_noise_pow_time, float *frame, NRState *st, boolean_T *speech_vad, float *y)
{
  ...
  boolean_T vad;
  float b_c[3];
  float tmp_fft_out[642];	/* fft_out[640] bin box order: [1].re, [2].re,[2].im, ... ,[320].re,[320].im,[321].re */
  float tmp_ifft_out[642];	/* ifft_out[640] bin box order: [1].re, [2].re,[2].im, ... ,[320].re,[320].im,[321].re */
