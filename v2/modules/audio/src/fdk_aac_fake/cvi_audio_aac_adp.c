...
//# aac dec lib
#if 0
typedef CVI_S32(*pCVI_AACDEC_GetVersion_Callback)(AACDEC_VERSION_S *pVersion);
#else
...
CVI_S32 OpenAACEncoder(CVI_VOID *pEncoderAttr, CVI_VOID **ppEncoder)
{
	...
	*ppEncoder = (CVI_VOID *)pstEncoder;
	...
	printf("[OpenAACEncoder][%d][%d][%d][%d][%d][%d]\n",
		config.coderFormat,
		config.bitRate,
...
CVI_S32 CVI_MPI_AENC_AacInit(CVI_VOID)
{
	...
	//printf("start register AAC encoder xxxxxxxxxxxxxxxNNN\n");
	//printf("PT_AAC_PRINT[%d]\n", (int)PT_AAC);
