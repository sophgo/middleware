...
#define S_CTRL_VALUE(_fd, _cfg, _ioctl)\
	do {\
		...
		ec1.id = _ioctl;\
		ec1.value = _cfg;\
		rc = ioctl(_fd, VI_IOC_S_CTRL, &ec1);\
		if (rc < 0) {\
...
#define G_CTRL_VALUE(_fd, _out, _ioctl)\
	do {\
		...
		ec1.id = _ioctl;\
		ec1.value = 0;\
		rc = ioctl(_fd, VI_IOC_G_CTRL, &ec1);\
		if (rc < 0) {\
...
{
	S_CTRL_PTR(fd, online, VI_IOCTL_SET_SC_ONLINE);
}

int vi_get_online2sc(int fd, struct cvi_isp_sc_online *online)
{
	G_CTRL_PTR(fd, online, VI_IOCTL_GET_SC_ONLINE);
}
...
int vi_set_dev_num(int fd, CVI_U32 devNum)
{
	S_CTRL_VALUE(fd, devNum, VI_IOCTL_SET_DEV_NUM);
}

int vi_get_dev_num(int fd, CVI_U32 *devNum)
{
	G_CTRL_VALUE(fd, devNum, VI_IOCTL_GET_DEV_NUM);
}
...
{
	SDK_CTRL_SET_CFG(fd, pstDevBindAttr, VI_SDK_SET_DEV_BIND_ATTR, dev, -1, -1, -1);
}
...
{
	SDK_CTRL_SET_CFG(fd, pstStatus, VI_SDK_GET_CHN_STATUS, -1, pipe, chn, -1);
}
...
{
	SDK_CTRL_SET_CFG(fd, NULL, VI_SDK_DISABLE_DEV, dev, -1, -1, -1);
}
...
{
	SDK_CTRL_SET_CFG(fd, NULL, VI_SDK_DESTROY_PIPE, -1, pipe, -1, -1);
}
...
{
	SDK_CTRL_SET_CFG(fd, NULL, VI_SDK_SET_BYPASS_FRM, -1, snr_num, -1, bypass_num);
}
...
{
	SDK_CTRL_SET_CFG(fd, pstDevTimingAttr, VI_SDK_GET_DEV_TIMING_ATTR, dev, -1, -1, -1);
}
...
{
	SDK_CTRL_SET_CFG(fd, source, VI_SDK_GET_PIPE_FRM_SRC, -1, pipe, -1, -1);
}
...
{
	SDK_CTRL_SET_CFG(fd, pstCropInfo, VI_SDK_SET_PIPE_CROP, -1, pipe, -1, -1);
}

int vi_sdk_get_pipe_crop(int fd, int pipe, CROP_INFO_S *pstCropInfo)
{
	SDK_CTRL_SET_CFG(fd, pstCropInfo, VI_SDK_GET_PIPE_CROP, -1, pipe, -1, -1);
}
...
{
	SDK_CTRL_SET_CFG(fd, cfg, VI_SDK_SET_CHN_ROTATION, -1, cfg->ViPipe, cfg->ViChn, -1);
}

int vi_sdk_get_chn_rotation(int fd, const struct vi_chn_rot_cfg *cfg)
{
	SDK_CTRL_SET_CFG(fd, cfg, VI_SDK_GET_CHN_ROTATION, -1, cfg->ViPipe, cfg->ViChn, -1);
}
...
{
	SDK_CTRL_SET_CFG(fd, cfg, VI_SDK_GET_CHN_LDC, -1, pipe, chn, -1);
}

int vi_sdk_set_chn_flip_mirror(int fd, const struct vi_chn_flip_mirror_cfg *cfg)
{
	SDK_CTRL_SET_CFG(fd, cfg, VI_SDK_SET_CHN_FLIP_MIRROR, -1, cfg->ViPipe, cfg->ViChn, -1);
}

int vi_sdk_get_chn_flip_mirror(int fd, struct vi_chn_flip_mirror_cfg *cfg)
{
	SDK_CTRL_SET_CFG(fd, cfg, VI_SDK_GET_CHN_FLIP_MIRROR, -1, cfg->ViPipe, cfg->ViChn, -1);
}
