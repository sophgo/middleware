...
#if defined(__CV180X__) || defined(__CV186X__)
...
CVI_S32 dump_register_181x(VI_PIPE ViPipe, FILE *fp, VI_DUMP_REGISTER_TABLE_S *pstRegTbl)
{
	VDEV_CLOSED_CHK(VDEV_TYPE_ISP, ViPipe);
	CHECK_VI_NULL_PTR(fp);
	CHECK_VI_NULL_PTR(pstRegTbl);
	...
	s32Ret = vi_sdk_get_dev_attr(fd, (int)ViPipe, &stDevAttr);
	if (s32Ret != CVI_SUCCESS) {
...
CVI_S32 dump_register_cv186x(VI_PIPE ViPipe, FILE *fp, VI_DUMP_REGISTER_TABLE_S *pstRegTbl)
{
	VDEV_CLOSED_CHK(VDEV_TYPE_ISP, ViPipe);
	CHECK_VI_NULL_PTR(fp);
	CHECK_VI_NULL_PTR(pstRegTbl);
	...
	fprintf(fp, "}");
	/* start tuning update */
	system("echo 0,0,0,0 > /sys/module/soph_vi/parameters/tuning_dis");
