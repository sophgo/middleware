...
CVI_S32 cvi_tde_rotate(CVI_S32 handle, const cvi_tde_single_src *single_src, cvi_tde_rotate_angle rotate)
{
	...
	offset_x = dst_rect->width / 2.0f;
	offset_y = dst_rect->height / 2.0f;
	center_x = offset_x + dst_rect->pos_x;
	center_y = offset_y + dst_rect->pos_y;

	switch(rotate) {
		case CVI_TDE_ROTATE_CLOCKWISE_90:
			deg = 90.0f;
			delta_x = ((vg_lite_float_t)src_rect->height - src_rect->width) / 2;
			delta_y = ((vg_lite_float_t)src_rect->width - src_rect->height) / 2;
			break;
	...
	CVI_TDE_DBG("center_x:%f, center_y:%f\n", center_x, center_y);
	CVI_TDE_DBG("offset_x:%f, offset_y:%f\n", offset_x, offset_y);
	CVI_TDE_DBG("delta_x:%f, delta_y:%f\n", delta_x, delta_y);
