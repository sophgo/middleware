...
SNS_COMBO_DEV_ATTR_S *g_pastSC1346_1LComboDevArray[VI_MAX_PIPE_NUM] = {CVI_NULL};
...
#define SC1346_1L_SENSOR_SET_COMBO(dev, pstCtx)   (g_pastSC1346_1LComboDevArray[dev] = pstCtx)
#define SC1346_1L_SENSOR_GET_COMBO(dev, pstCtx)   (pstCtx = g_pastSC1346_1LComboDevArray[dev])
...
ISP_SNS_COMMADDR_U g_aunSC1346_1L_AddrInfo[VI_MAX_PIPE_NUM] = {
	[0] = { .s8I2cAddr = 0},
...
static CVI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_SYNC_INFO_S *pstSnsSyncInfo)
{
	...
	if ((pstSnsState->bSyncInit == CVI_FALSE) || (pstSnsRegsInfo->bConfig == CVI_FALSE)) {
		...
		for (i = 0; i < pstCfg0->snsCfg.u32RegNum; i++) {
			...
			pstI2c_data[i].u8DevAddr = g_aunSC1346_1L_AddrInfo[ViPipe].s8I2cAddr;
...
static CVI_S32 sensor_rx_attr(VI_PIPE ViPipe, SNS_COMBO_DEV_ATTR_S *pstRxAttr)
{
	...
	SC1346_1L_SENSOR_GET_CTX(ViPipe, pstSnsState);
	SC1346_1L_SENSOR_GET_COMBO(ViPipe, pstRxAttrSrc);
	...
	pstRxAttr->img_size.start_x = g_astSC1346_1L_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.s32X;
	pstRxAttr->img_size.start_y = g_astSC1346_1L_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.s32Y;
	pstRxAttr->img_size.active_w = g_astSC1346_1L_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.u32Width;
	pstRxAttr->img_size.active_h = g_astSC1346_1L_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.u32Height;
	...
	pstRxAttr->img_size.max_width = g_astSC1346_1L_mode[pstSnsState->u8ImgMode].astImg[0].stMaxSize.u32Width;
	pstRxAttr->img_size.max_height = g_astSC1346_1L_mode[pstSnsState->u8ImgMode].astImg[0].stMaxSize.u32Height;
	if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
...
static CVI_S32 sensor_patch_rx_attr(VI_PIPE ViPipe, RX_INIT_ATTR_S *pstRxInitAttr)
{
	...
	if (!g_pastSC1346_1LComboDevArray[ViPipe]) {
		pstRxAttr = malloc(sizeof(SNS_COMBO_DEV_ATTR_S));
	} else {
		SC1346_1L_SENSOR_GET_COMBO(ViPipe, pstRxAttr);
	}
	memcpy(pstRxAttr, &sc1346_1L_rx_attr, sizeof(SNS_COMBO_DEV_ATTR_S));
	SC1346_1L_SENSOR_SET_COMBO(ViPipe, pstRxAttr);
...
{
	if (g_pastSC1346_1LComboDevArray[ViPipe]) {
		free(g_pastSC1346_1LComboDevArray[ViPipe]);
		g_pastSC1346_1LComboDevArray[ViPipe] = CVI_NULL;
	}
...
static CVI_VOID sensor_patch_i2c_addr(VI_PIPE ViPipe, CVI_S32 s32I2cAddr)
{
	if (SC1346_1L_I2C_ADDR_IS_VALID(s32I2cAddr))
		g_aunSC1346_1L_AddrInfo[ViPipe].s8I2cAddr = s32I2cAddr;
	else {
		CVI_TRACE_SNS(CVI_DBG_ERR, "I2C addr input error ,please check [0x%x]\n", s32I2cAddr);
		g_aunSC1346_1L_AddrInfo[ViPipe].s8I2cAddr = SC1346_1L_I2C_ADDR_2;
	}
}
