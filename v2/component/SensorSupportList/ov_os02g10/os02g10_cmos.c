...
SNS_COMBO_DEV_ATTR_S* g_pastOs02g10ComboDevArray[VI_MAX_PIPE_NUM] = {CVI_NULL};
...
#define OS02G10_SENSOR_SET_COMBO(dev, pstCtx)   (g_pastOs02g10ComboDevArray[dev] = pstCtx)
#define OS02G10_SENSOR_GET_COMBO(dev, pstCtx)   (pstCtx = g_pastOs02g10ComboDevArray[dev])
...
ISP_SNS_COMMADDR_U g_aunOs02g10_AddrInfo[VI_MAX_PIPE_NUM] = {
	[0] = { .s8I2cAddr = 0},
...
static CVI_S32 sensor_rx_attr(VI_PIPE ViPipe, SNS_COMBO_DEV_ATTR_S *pstRxAttr)
{
	...
	OS02G10_SENSOR_GET_CTX(ViPipe, pstSnsState);
	OS02G10_SENSOR_GET_COMBO(ViPipe, pstRxAttrSrc);
	CMOS_CHECK_POINTER(pstSnsState);
	...
	pstRxAttr->img_size.start_x = g_astOs02g10_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.s32X;
	pstRxAttr->img_size.start_y = g_astOs02g10_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.s32Y;
	pstRxAttr->img_size.active_w = g_astOs02g10_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.u32Width;
	pstRxAttr->img_size.active_h = g_astOs02g10_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.u32Height;
	...
	pstRxAttr->img_size.max_width = g_astOs02g10_mode[pstSnsState->u8ImgMode].astImg[0].stMaxSize.u32Width;
	pstRxAttr->img_size.max_height = g_astOs02g10_mode[pstSnsState->u8ImgMode].astImg[0].stMaxSize.u32Height;
...
static CVI_S32 sensor_patch_rx_attr(VI_PIPE ViPipe, RX_INIT_ATTR_S *pstRxInitAttr)
{
	...
	if (!g_pastOs02g10ComboDevArray[ViPipe]) {
		pstRxAttr = malloc(sizeof(SNS_COMBO_DEV_ATTR_S));
	} else {
		OS02G10_SENSOR_GET_COMBO(ViPipe, pstRxAttr);
	}
	memcpy(pstRxAttr, &os02g10_rx_attr, sizeof(SNS_COMBO_DEV_ATTR_S));
	OS02G10_SENSOR_SET_COMBO(ViPipe, pstRxAttr);
...
{
	if (g_pastOs02g10ComboDevArray[ViPipe]) {
		free(g_pastOs02g10ComboDevArray[ViPipe]);
		g_pastOs02g10ComboDevArray[ViPipe] = CVI_NULL;
	}
...
static CVI_VOID sensor_patch_i2c_addr(VI_PIPE ViPipe, CVI_S32 s32I2cAddr)
{
	if (OS02G10_I2C_ADDR_IS_VALID(s32I2cAddr))
		g_aunOs02g10_AddrInfo[ViPipe].s8I2cAddr = s32I2cAddr;
	else {
		CVI_TRACE_SNS(CVI_DBG_ERR, "I2C addr input error ,please check [0x%x]\n", s32I2cAddr);
		g_aunOs02g10_AddrInfo[ViPipe].s8I2cAddr = OS02G10_I2C_ADDR;
	}
}
...
static CVI_VOID sensor_ctx_exit(VI_PIPE ViPipe)
{
	...
	g_aeOs02g10_MirrorFlip[ViPipe] = ISP_SNS_NORMAL;
}
