...
SNS_COMBO_DEV_ATTR_S *g_pasttp2825ComboDevArray[VI_MAX_PIPE_NUM] = {CVI_NULL};
...
#define TP2825_SENSOR_GET_COMBO(dev, pstCtx)   (pstCtx = g_pasttp2825ComboDevArray[dev])
#define TP2825_SENSOR_SET_COMBO(dev, pstCtx)   (g_pasttp2825ComboDevArray[dev] = pstCtx)

#define TP2825_RES_IS_1440P(w, h)      ((w) == 2560 && (h) == 1440)
#define TP2825_RES_IS_1080P(w, h)      ((w) == 1920 && (h) == 1080)
#define TP2825_RES_IS_720P(w, h)      ((w) == 1280 && (h) == 720)
#define TP2825_FPS_IS_25P(f)      (f == 25)
#define TP2825_FPS_IS_30P(f)      (f == 30)
#define TP2825_FPS_IS_50P(f)      (f == 50)
...
static CVI_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
	...
	if (TP2825_RES_IS_1440P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
		if (TP2825_FPS_IS_25P(pstSensorImageMode->f32Fps)) {
			u8SensorImageMode = TP2825_MODE_1440P_25P;
		} else {
			u8SensorImageMode = TP2825_MODE_1440P_30P;
		}
	} else if(TP2825_RES_IS_720P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
			u8SensorImageMode = TP2825_MODE_720P_30P;
	} else if (TP2825_RES_IS_1080P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
		if (TP2825_FPS_IS_50P(pstSensorImageMode->f32Fps)) {
			u8SensorImageMode = TP2825_MODE_1080P_50P;
		} else if (TP2825_FPS_IS_30P(pstSensorImageMode->f32Fps)) {
			u8SensorImageMode = TP2825_MODE_1080P_30P;
		} else {
			u8SensorImageMode = TP2825_MODE_1080P_60P;
		}
	} else {
...
static CVI_S32 sensor_rx_attr(VI_PIPE ViPipe, SNS_COMBO_DEV_ATTR_S *pstRxAttr)
{
	...
	TP2825_SENSOR_GET_CTX(ViPipe, pstSnsState);
	TP2825_SENSOR_GET_COMBO(ViPipe, pstRxAttrSrc);
	...
	pstRxAttr->img_size.start_x = g_asttp2825_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.s32X;
	pstRxAttr->img_size.start_y = g_asttp2825_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.s32Y;
	pstRxAttr->img_size.active_w = g_asttp2825_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.u32Width;
	pstRxAttr->img_size.active_h = g_asttp2825_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.u32Height;
	...
	pstRxAttr->img_size.max_width = g_asttp2825_mode[pstSnsState->u8ImgMode].astImg[0].stMaxSize.u32Width;
	pstRxAttr->img_size.max_height = g_asttp2825_mode[pstSnsState->u8ImgMode].astImg[0].stMaxSize.u32Height;
...
static CVI_S32 sensor_patch_rx_attr(VI_PIPE ViPipe, RX_INIT_ATTR_S *pstRxInitAttr)
{
	...
	if (!g_pasttp2825ComboDevArray[ViPipe]) {
		pstRxAttr = malloc(sizeof(SNS_COMBO_DEV_ATTR_S));
	} else {
		TP2825_SENSOR_GET_COMBO(ViPipe, pstRxAttr);
	}
	memcpy(pstRxAttr, &tp2825_rx_attr, sizeof(SNS_COMBO_DEV_ATTR_S));
	TP2825_SENSOR_SET_COMBO(ViPipe, pstRxAttr);
