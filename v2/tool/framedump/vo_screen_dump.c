...
void VO_Screen_Dump_HandleSig(CVI_S32 signo)
{
	if (u32SignalFlag) {
		exit(-1);
	}
	...
	if (SIGINT == signo || SIGTERM == signo) {
		...
		if (stFrameInfo.stVFrame.u64PhyAddr[0])
			CVI_VO_ReleaseScreenFrame(VoLayer, &stFrameInfo);
		u32SignalFlag--;
...
CVI_VOID SAMPLE_MISC_VoScreenDump(VO_LAYER Layer, CVI_U32 u32FrameCnt)
{
	CVI_S32 s32Ret = 0;
	...
	/* get frame  */
	while (u32Cnt--) {
		if (CVI_VO_GetScreenFrame(Layer, &stFrameInfo, s32MilliSec) != CVI_SUCCESS) {
		...
		if (bFlag) {
			...
			snprintf(szFrameName, 128, "./vo_layer%d_screen_%dx%d_%s_%d.yuv", Layer,
					 stFrameInfo.stVFrame.u32Width, stFrameInfo.stVFrame.u32Height,
					 szPixFrm, u32FrameCnt);
			printf("Dump frame of vo screen to file: \"%s\"\n", szFrameName);
			pfd = fopen(szFrameName, "wb");
			if (CVI_NULL == pfd) {
				printf("open file failed:%s!\n", strerror(errno));
				CVI_VO_ReleaseScreenFrame(Layer, &stFrameInfo);
				return;
			}
			bFlag = CVI_FALSE;
		}
		...
		if (CVI_VO_ReleaseScreenFrame(Layer, &stFrameInfo) != CVI_SUCCESS)
			printf("CVI_VO_ReleaseScreenFrame fail\n");
...
static void usage(void)
{
	printf(
		...
		"*)Example:\n"
		"	e.g : ./vo_screen_dump 0 1\n"
		"	e.g : ./vo_screen_dump 1 2\n"
		"*************************************************\n"
		"\n");
}
...
int main(int argc, char **argv)
{
	...
	u32SignalFlag = 0;
	signal(SIGINT, VO_Screen_Dump_HandleSig);
	signal(SIGTERM, VO_Screen_Dump_HandleSig);

	u32FrmCnt = atoi(argv[2]);/* frame count*/
    printf("VoLayer=%d, u32FrmCnt=%d, starting dump screen frame...\n", VoLayer, u32FrmCnt);
	SAMPLE_MISC_VoScreenDump(VoLayer, u32FrmCnt);
