...
void VI_Chn_Dump_HandleSig(CVI_S32 signo)
{
	if (u32SignalFlag) {
		exit(-1);
	}
...
CVI_VOID SAMPLE_MISC_ViDump(VI_PIPE Pipe, VI_CHN Chn, CVI_U32 u32FrameCnt)
{
	CVI_S32 s32Ret = 0;
	...
	/* get frame  */
	while (u32Cnt--) {
		if (CVI_VI_GetChnFrame(Pipe, Chn, &stFrameInfo, s32MilliSec) != CVI_SUCCESS) {
		...
		if (bFlag) {
			...
			snprintf(szFrameName, 128, "./vi_pipe%d_chn%d_%dx%d_%s_%d.yuv", Pipe, Chn,
					 stFrameInfo.stVFrame.u32Width, stFrameInfo.stVFrame.u32Height,
					 szPixFrm, u32FrameCnt);
			printf("Dump frame of vi chn %d to file: \"%s\"\n", Chn, szFrameName);
			pfd = fopen(szFrameName, "wb");
			if (CVI_NULL == pfd) {
				printf("open file failed:%s!\n", strerror(errno));
				CVI_VI_ReleaseChnFrame(Pipe, Chn, &stFrameInfo);
				return;
			}
			bFlag = CVI_FALSE;
		}
		...
		if (CVI_VI_ReleaseChnFrame(Pipe, Chn, &stFrameInfo) != CVI_SUCCESS)
...
static void usage(void)
{
	printf(
		"\n"
		"*************************************************\n"
		"Usage: ./vi_chn_dump [ViPipe] [ViChn] [FrmCnt]\n"
		"1)ViPipe: \n"
		"	Vi pipe id\n"
		"2)ViChn: \n"
		"	vi chn id\n"
...
int main(int argc, char **argv)
{
	...
	SAMPLE_MISC_ViDump(ViPipe, ViChn, u32FrmCnt);
