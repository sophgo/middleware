...
int test_cannyedge(int bTileMode, int bWrite, int bInstant)
{
	...
	if (bTileMode) {
		...
		strcpy(output_file_3x3, "sample_tile_CannyEdge_3x3.yuv");
		strcpy(output_file_5x5, "sample_tile_CannyEdge_5x5.yuv");
	} else {
		input_w = 352;
		input_h = 288;
		...
		strcpy(output_file_3x3, "sample_CannyEdge_3x3.yuv");
		strcpy(output_file_5x5, "sample_CannyEdge_5x5.yuv");
	}
	/* 5 by 5*/
	...
	//CVI_IVE_CannyEdge(handle, &src, &stEdge, &stCannyHysEdgeCtrl, bInstant);
	CVI_IVE_CannyHysEdge(handle, &src, &stEdge, &stStack,
			     &stCannyHysEdgeCtrl, bInstant);
	CVI_IVE_CannyEdge(&stEdge, &stStack);
