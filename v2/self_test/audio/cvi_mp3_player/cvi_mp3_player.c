...
void *AO_SPEED_CHANGE_PROC(void *pt_arg)
{
#define BUFF_LEN 128
	pt_arg = pt_arg;
	printf("[%s]---->\n", __func__);
	char str[BUFF_LEN];
	char keyword[20] = "speed";
	ST_CVIAO_SPEEDPLAY_CONFIG stAoSpeedCfg;
	bool bSpeedPlay = false;
	int s32Ret;
	//using example:
	//https://blog.csdn.net/CJ_Kano/article/details/41790157
	memset(&stAoSpeedCfg, 0, sizeof(ST_CVIAO_SPEEDPLAY_CONFIG));
	while (1) {
		sleep(3);
		fgets(str, BUFF_LEN, stdin);
		if (strncmp(str, keyword, 5) == 0) {
			bSpeedPlay = true;
			if (bSpeedPlay == true) {
				printf("enter sample rate:\n");
				scanf("%d", &stAoSpeedCfg.sampleRate);
				printf("enter channels:\n");
				scanf("%d", &stAoSpeedCfg.channels);
				printf("enter speed:\n");
				scanf("%f", &stAoSpeedCfg.speed);
				printf("enter pitch:\n");
				scanf("%f", &stAoSpeedCfg.pitch);
				printf("enter rate:\n");
				scanf("%f", &stAoSpeedCfg.rate);
				printf("enter volume:\n");
				scanf("%f", &stAoSpeedCfg.volume);
				s32Ret = CVI_AO_EnableSpeedPlay(0, stAoSpeedCfg);
				if (s32Ret != CVI_SUCCESS)
					printf("speed play setup failure\n");
				else
					printf("speed play setup success!!!!!\n");

			}
				printf("Do not acquire speedplay in aout\n");
		} else {
			printf("disable speed playxxxxxxxxxxx[%s]\n", str);
			bSpeedPlay = false;
			sleep(1);
		}
...
#ifdef USE_SONIC_LIB
	//create a thread -- to catch the phrase "speed"
	pthread_create(&stTestModeThread, 0, AO_SPEED_CHANGE_PROC, NULL);
