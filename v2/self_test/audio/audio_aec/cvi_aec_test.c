...
#define SAMPLE_AUDIO_MIPS_AEC_AES_NR_AGC_MAX 10
	...
	if (use_default_parameters > SAMPLE_AUDIO_AEC_USER_PARAM) {
		...
		if (use_default_parameters == SAMPLE_AUDIO_MIPS_AEC_MIN) {
			...
			default_AGC_Setting.para_agc_vad_ena = 0;
			ploadhandle = CVI_AUDIO_LoadingCheck_Init(sr, 2, load_check_sec, "NR");
		} else if (use_default_parameters == SAMPLE_AUDIO_MIPS_AGC) {
			pstAiVqeAttr.u32OpenMask = AGC_ENABLE;
			default_AGC_Setting.para_agc_vad_ena = 0;
			ploadhandle = CVI_AUDIO_LoadingCheck_Init(sr, 2, load_check_sec, "AGC");
	...
	s32MacSize = AEC_INPUT_PERIOD_SIZE * sizeof(CVI_S16); //one channel output
	...
	while (1) {
		...
		#if 0
		printf("[%s][%d] enter size in bytes[%d]\n", __func__, __LINE__,
			pstAiVqeAttr.s32BytesPerSample * pstAiVqeAttr.s32FrameSample);
		#endif
