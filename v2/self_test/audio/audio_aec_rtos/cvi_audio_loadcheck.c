...
int CVI_AUDIO_LoadingCheck_End(void *phandle, unsigned int bytes_proceed)
{
	...
	if (pLoadCheck->CurCnt - pLoadCheck->DataIntervalCnt == pLoadCheck->MeasureInterval) {
		...
		duration = (float)(tv2.tv_sec - tv1.tv_sec) * 1000 +
				(float)(tv2.tv_usec - tv1.tv_usec)/1000;

		//if ((time_tick_diff)/pLoadCheck->MeasureInterval != 0) {
		if ((duration)/pLoadCheck->MeasureInterval != 0) {
			//printf("name[%s]->bytes[%d] time interval[%f]ticks cpu[%ld]%% measure every[%d]sec\n",
			printf("name[%s]->bytes[%d] time interval[%f]ms cpu[%f]ms measure every[%d]sec\n",
			pLoadCheck->name,
			pLoadCheck->OneSecData,
			duration,
			((duration / pLoadCheck->MeasureInterval)/1000)*100,
			//time_tick_diff,
