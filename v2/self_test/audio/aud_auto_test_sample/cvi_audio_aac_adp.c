		     ...
		     CVI_S32 s32InputSizeBytes, CVI_U32 *pu32OutLen)
{
	...
	if (!pEncoder || !inputdata || !pu8Outbuf || !pu32OutLen) {
		printf("[Func]:%s [Line]:%d ptr is null\n", __func__, __LINE__);
		return -1;
	}
		     ...
		     CVI_U16 *pu16Outbuf, CVI_U32 *pu32OutLen, CVI_U32 *pu32Chns)
{
	...
	if (!pDecoder || !pu8Inbuf || !ps32LeftByte || !pu16Outbuf) {
		printf("[Func]:%s [Line]:%d ptr is null\n", __func__, __LINE__);
		return -1;
	}
	if (!pu32OutLen || !pu32Chns) {
		printf("[Func]:%s [Line]:%d ptr is null\n", __func__, __LINE__);
		return -1;
	}
