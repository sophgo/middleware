...
void JpgEncSetQualityFactor(JpgEncHandle handle, Uint32 quality, BOOL useStdTable)
{
    ...
    */
    ...
    for (i = 0; i<64; i++) {
        pLumaQTable[i] = (useStdTable == TRUE) ? std_luminance_quant_tbl[jpeg_natural_order[i]] : (Int32)pEncInfo->pQMatTab[DC_TABLE_INDEX0][i];
    }
    for (i = 0; i<64; i++) {
        pChromaQTable[i] = (useStdTable == TRUE) ? std_chrominance_quant_tbl[jpeg_natural_order[i]] : (Int32)pEncInfo->pQMatTab[AC_TABLE_INDEX0][i];
    }
