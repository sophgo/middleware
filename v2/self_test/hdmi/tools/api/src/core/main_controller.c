...
#include "main_controller/main_controller_reg.h"
...
{
	LOG_TRACE1(bit);
	printf("hdcp_clock disable\n");
	dev_write_mask(dev, MC_CLKDIS, MC_CLKDIS_HDCPCLK_DISABLE_MASK, 1);
}
...
{
	LOG_TRACE1(bit);
	dev_write_mask(dev, MC_SWRSTZREQ, MC_SWRSTZREQ_ISPDIFSWRST_REQ_MASK, bit);
}

void mc_audio_i2s_reset(hdmi_tx_dev_t *dev, u8 bit)
{
	LOG_TRACE1(bit);
	dev_write_mask(dev, MC_SWRSTZREQ, MC_SWRSTZREQ_II2SSWRST_REQ_MASK, bit);
}
...
{
	LOG_TRACE1(bit);
	dev_write_mask(dev, MC_SWRSTZREQ, MC_SWRSTZREQ_PIXELSWRST_REQ_MASK, bit);
}
...
#endif
	LOGGER(SNPS_WARN,"Heac Phy Reset not supported");
}
