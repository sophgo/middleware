...
int irq_unmute_source(hdmi_tx_dev_t *dev, irq_sources_t irq_source)
{
	...
	for(i = 0; irq_vec[i].source != 0; i++){
		if(irq_vec[i].source == irq_source){
			LOGGER(SNPS_DEBUG, "IRQ write unmute: irq[%d] mask[%d]", irq_source, 0x00);
...
{
	u32 mask;
    printf("irq_hpd_sense_enable1,line\n");
	i2cddc_fast_mode(dev, 0);
	printf("irq_hpd_sense_enable2\n");
	// Enable HDMI TX PHY HPD Detector
	phy_enable_hpd_sense(dev, TRUE);
	printf("irq_hpd_sense_enable3\n");
	...
	irq_unmask_bit(dev, PHY, mask);
	printf("irq_hpd_sense_enable4,line\n");
	...
	phy_interrupt_enable(dev, ~mask);
	printf("irq_hpd_sense_enable5,line\n");
	// Un-mask main interrupt
	irq_unmute(dev);
	printf("irq_hpd_sense_enable6\n");
}
