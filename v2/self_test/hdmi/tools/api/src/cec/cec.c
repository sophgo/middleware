 ...
 */
int cec_msgRx(hdmi_tx_dev_t * dev, char *buf, unsigned size,
		int timeout)
{
	int i, retval = -1;

	LOGGER(SNPS_TRACE, "%u, %i", size, timeout);
 ...
 */
int cec_msgTx(hdmi_tx_dev_t * dev, const char *buf, unsigned size,
		unsigned retry)
{
	int i, retval = -1;

	LOGGER(SNPS_DEBUG, "%u, %i", size, retry);
	if (size > 0) {
		if (cec_CfgTxBuf(dev, buf, size) == (int)size) {
			for (i = 0; i < (int)retry; i++) {
				if (cec_CfgSignalFreeTime(dev, i ? 5 : 7))
					break;
...
int cec_CfgLogicAddr(hdmi_tx_dev_t * dev, unsigned addr, int enable)
{
	...
	LOGGER(SNPS_TRACE,"%u, %i", addr, enable);

	if (addr > BCST_ADDR) {
		LOGGER(SNPS_TRACE,"invalid parameter");
		return -1;
	}
	if (addr == BCST_ADDR) {
		if (enable) {
			...
			LOGGER(SNPS_TRACE,"cannot de-allocate broadcast logical address");
			return -1;
		}
	}
...
{
	LOGGER(SNPS_TRACE,"%i", enable);
	if (enable)
		dev_write(dev, CEC_CTRL,dev_read(dev, CEC_CTRL) |  CEC_CTRL_STANDBY_MASK);
	else
		dev_write(dev, CEC_CTRL,dev_read(dev, CEC_CTRL) & ~CEC_CTRL_STANDBY_MASK);
	return 0;
}
...
{
	LOGGER(SNPS_TRACE,"0x%02X", mask);
	dev_write(dev, IH_MUTE_CEC_STAT0, dev_read(dev, IH_MUTE_CEC_STAT0) | mask);
...
int cec_CfgTxBuf(hdmi_tx_dev_t * dev, const char *buf, unsigned size)
{
	...
	{
		/* check if addressing is shifted by 2 in the setup and compensates */
		if (CEC_TX_DATA == 0x7D10)
		{
			dev_write(dev, CEC_TX_DATA + i, buf[i]);
		}
		else
		{
			LOGGER(SNPS_TRACE,"buf[%d] = %x\n", i, buf[i]);
			dev_write(dev, CEC_TX_DATA + (i<<2), buf[i]);
		}
	}
	return size;
}
...
int cec_CfgRxBuf(hdmi_tx_dev_t * dev, char *buf, unsigned size)
{
	...
	if (cnt > CEC_RX_DATA_SIZE) {
		LOGGER(SNPS_TRACE,"wrong byte count");
		return -1;
	}
	for (i = 0; i < cnt ; i++)
	{
		/* check if addressing is shifted by 2 in the setup and compensates */
		if (CEC_RX_DATA == 0x7D20)
			buf[i] = dev_read(dev, CEC_RX_DATA + i);
		else
			buf[i] = dev_read(dev, CEC_RX_DATA + (i<<2));
	}
	return cnt;
}
...
{
	LOGGER(SNPS_TRACE,"");
	dev_write(dev, CEC_LOCK, dev_read(dev, CEC_LOCK) & ~CEC_LOCK_LOCKED_BUFFER_MASK);
	return TRUE;
}
...
int cec_CfgBroadcastNAK(hdmi_tx_dev_t * dev, int enable)
{
	LOGGER(SNPS_TRACE,"%i", enable);
	if (enable)
		dev_write(dev, CEC_CTRL, dev_read(dev, CEC_CTRL) |  CEC_CTRL_BC_NACK_MASK);
	else
		dev_write(dev, CEC_CTRL, dev_read(dev, CEC_CTRL) & ~CEC_CTRL_BC_NACK_MASK);
	return TRUE;
}
		...
		unsigned src, unsigned dst)
{
	...
	LOGGER(SNPS_DEBUG, "%u, %u, %u", size, src, dst);
	...
	for (i = 0; i < (int)size; i++)
		tmp[i + 1] = buf[i];
	LOGGER(SNPS_DEBUG," %d\n", tmp[0]);
	if (cec_msgTx(dev, tmp, size + 1, 2) < 0)
		return -1;
	else
		return size;
}
