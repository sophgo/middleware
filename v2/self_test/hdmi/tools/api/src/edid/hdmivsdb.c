...
int hdmivsdb_parse(hdmi_tx_dev_t *dev, hdmivsdb_t * vsdb, u8 * data)
{
	...
	if (blockLength < 5) {
		LOGGER(SNPS_ERROR,"%s Invalid minimum length (%d)", __func__, blockLength);
		return FALSE;
	}
	...
	if (bit_field(data[8], 5, 1) == 1) {	/* additional video format capabilities are described */
		...
		LOGGER(SNPS_DEBUG,"HDMI VIC Count %d", hdmiVicLen);
		LOGGER(SNPS_DEBUG,"HDMI 3D Present %d", bit_field(data[videoInfoStart], 7, 1));
			...
			if (hdmi3dLen > spanned3d) {
				...
				for (i = 0, j = 0; i < (hdmi3dLen - spanned3d); i++) {
...
#endif
					LOGGER(SNPS_DEBUG,"3D struct %d - %d", bit_field(data[hdmi3dStart + i + j], 4, 4), bit_field(data[hdmi3dStart + i + j], 0, 4));
					...
					if (bit_field(data[hdmi3dStart + i + j], 0, 4) > 7) {	/* bytes with 3D_Detail_X and Reserved(0) are present only when 3D_Structure_X > b'1000 - side-by-side(half) */
						j++;
						LOGGER(SNPS_DEBUG,"3D_Detail_X struct %d - %d - %d", bit_field(data[hdmi3dStart + i + j], 4, 4), bit_field(data[hdmi3dStart + i + j], 4, 4), bit_field(data[hdmi3dStart + i + j], 4, 4));
