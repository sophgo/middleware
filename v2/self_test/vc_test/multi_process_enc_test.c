...
int ChildProcessGetEncodedStream(chnInputCfg *pIc)
{
    ...
    fd = CVI_VENC_GetFd(VeChn);
    while(s32NumFrames && s32RetryCnt > 0) {
        FD_ZERO(&read_fds);
        FD_SET(fd, &read_fds);
        TimeoutVal.tv_sec  = 2;
...
int ParentProcessSendEncodeFrame(chnInputCfg *pIc)
{
    VENC_CHN VeChn = pIc->s32Chn;
    ...
    memset(stFrame.stVFrame.pu8VirAddr[0], 0, stFrame.stVFrame.u32Length[0]);
    while(s32NumFrames)
    {
...
int main(int argc, char *argv[])
{
    ...
    printf("parent process: codec:%s width:%d height:%d input:%s output file:%s\n",
        pChnIc->codec, pChnIc->width, pChnIc->height, pChnIc->input_path, pChnIc->output_path);

    pid_t pid = fork();
    if (pid == 0) {
        ChildProcessGetEncodedStream(pChnIc);
    } else if (pid > 0) {
        ParentProcessSendEncodeFrame(pChnIc);
        wait(NULL);
    } else {
        perror("fork");
        exit(1);
