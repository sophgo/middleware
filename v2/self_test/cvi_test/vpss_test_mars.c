...
// vpss_fmt_input_test, output format, 4tap
static CVI_CHAR *filenames_out[] = {
	"res/vpss_fmt_in.bin",
	"res/vpss_fmt_in.bin",
	"res/vpss_fmt_in.bin",
	"res/vpss_fmt_in.bin",
	"res/vpss_fmt_in_yuv420.bin",	/* YUV420 -> RGB */
	"res/vpss_fmt_in_yuv422.bin",
	"res/vpss_fmt_in_y.bin",
	"res/vpss_fmt_in_yuv420.bin",	/* NV12 -> RGB */
	"res/vpss_fmt_in_yuv420.bin",
	"res/vpss_fmt_in_yuv422.bin",
	"res/vpss_fmt_in_yuv422.bin",
	"res/vpss_fmt_in_yuv422.bin",
	"res/vpss_fmt_in_yuv422.bin",
...
// vpss_fmt_input_test, output format, 2tap
static CVI_CHAR *filenames_out_2tap[] = {
	"res/vpss_fmt_in_2tap.bin",
	"res/vpss_fmt_in_2tap.bin",
	"res/vpss_fmt_in_2tap.bin",
	"res/vpss_fmt_in_2tap.bin",
	"res/vpss_fmt_in_yuv420_2tap.bin",	/* YUV420 -> RGB */
	"res/vpss_fmt_in_yuv422_2tap.bin",
	"res/vpss_fmt_in_y_2tap.bin",
	"res/vpss_fmt_in_yuv420_2tap.bin",	/* NV12 -> RGB */
	"res/vpss_fmt_in_yuv420_2tap.bin",
	"res/vpss_fmt_in_yuv422_2tap.bin",
	"res/vpss_fmt_in_yuv422_2tap.bin",
	"res/vpss_fmt_in_yuv422_2tap.bin",
	"res/vpss_fmt_in_yuv422_2tap.bin",
...
// vpss_fmt_output_test, output format
static PIXEL_FORMAT_E filefmt_out[] = {
	PIXEL_FORMAT_RGB_888,
	PIXEL_FORMAT_BGR_888,
	PIXEL_FORMAT_RGB_888_PLANAR,
	PIXEL_FORMAT_BGR_888_PLANAR,

	PIXEL_FORMAT_YUV_PLANAR_420,	// 4, YUV420 planar I420
	PIXEL_FORMAT_YUV_PLANAR_422,	// 5, YUV422 planar YV16
	PIXEL_FORMAT_YUV_400,

	PIXEL_FORMAT_HSV_888,	// 7, rawViewer.py, HSV_packed(9)
	PIXEL_FORMAT_HSV_888_PLANAR,	// 8, rawViewer.py, HSV(8)

	PIXEL_FORMAT_NV12,	// 9, YUV420 planar NV12
	PIXEL_FORMAT_NV21,	// 10, YUV42 planr NV21
	PIXEL_FORMAT_NV16,	// 11, rawViewer.py, NV16(13)
	PIXEL_FORMAT_NV61,	// 12, rawViewer.py, NV61(14)
	PIXEL_FORMAT_YUYV,	// 13, YUV422 YUVY
	PIXEL_FORMAT_YVYU,	// 14, YUV422 YUYV, swap UV
	PIXEL_FORMAT_UYVY,	// 15, YUV422, VYUY, swap UV
	PIXEL_FORMAT_VYUY,	// 16, YUV422, VYUY
};

static CVI_S32 _MARS_VPSS_VI_TEST(SIZE_S stSize, bool fail_pause)
{
...
static CVI_S32 _MARS_VPSS_FMT_INPUT_TEST(bool fail_pause)
{
	...
	VIDEO_FRAME_INFO_S stVideoFrame;
	CVI_CHAR filename[64], *filename_out;
	CVI_S32 s32Ret = CVI_SUCCESS;
	...
	for (CVI_U32 i = 0; i < ARRAY_SIZE(filefmt); ++i) {
		...
		if (VpssChn == 1) {
			/* Only sc_v1 is 4tap */
			filename_out = filenames_out[i];
			snprintf(filename, 32, "./res/vpss_fmt_in_ng_%d.bin", i);
		} else {
			/* 2tap */
			filename_out = filenames_out_2tap[i];
			snprintf(filename, 64, "./res/vpss_fmt_in_2tap_ng_%d.bin", i);
		}
		if (!SAMPLE_COMM_FRAME_CompareWithFile(filename_out, &stVideoFrame)) {
...
static CVI_S32 _MARS_VPSS_FMT_OUTPUT_TEST(bool fail_pause)
{
	SIZE_S stSize = {.u32Width = 1920, .u32Height = 1080};
	VPSS_GRP_ATTR_S    stVpssGrpAttr;
	VPSS_GRP	   VpssGrp = 0;
	...
	CVI_S32 s32Ret = CVI_SUCCESS;
	bool is_result_match = CVI_TRUE;
	const int fmt_in_idx = 4; // yuv420

	// VpssGrp 0
	stVpssGrpAttr.stFrameRate.s32SrcFrameRate    = -1;
	stVpssGrpAttr.stFrameRate.s32DstFrameRate    = -1;
	stVpssGrpAttr.enPixelFormat		     = filefmt[fmt_in_idx];
	stVpssGrpAttr.u32MaxW			     = stSize.u32Width;
	stVpssGrpAttr.u32MaxH			     = stSize.u32Height;
	...
	for (CVI_U32 i = 0; i < ARRAY_SIZE(filefmt_out); ++i) {
		...
		CVI_TRACE_LOG(CVI_DBG_INFO, "filefmt(%d) filename(%s)\n", filefmt[fmt_in_idx], filenames[fmt_in_idx]);
		SAMPLE_COMM_VPSS_SendFrame(VpssGrp, &stSize, filefmt[fmt_in_idx], filenames[fmt_in_idx]);
		s32Ret = CVI_VPSS_GetChnFrame(VpssGrp, VpssChn, &stVideoFrame, 1000);
		if (s32Ret != CVI_SUCCESS) {
			CVI_TRACE_LOG(CVI_DBG_ERR, "CVI_VPSS_GetChnFrame for grp%d chn%d. s32Ret: 0x%x !\n"
				, VpssGrp, VpssChn, s32Ret);
		...
		/* Only sc_v1 is 4tap */
		if (VpssChn == 1)
			snprintf(filename, 64, "./res/vpss_fmt_out_%d.bin", i);
		else
			snprintf(filename, 64, "./res/vpss_fmt_out_%d_2tap.bin", i);
		if (!SAMPLE_COMM_FRAME_CompareWithFile(filename, &stVideoFrame)) {
			CVI_TRACE_LOG(CVI_DBG_ERR, "output test(%d) format(%d) mismatch!\n", i, filefmt_out[i]);
			is_result_match = CVI_FALSE;

			if (VpssChn == 1)
				snprintf(filename, 64, "./res/vpss_fmt_out_ng_%d.bin", i);
			else
				snprintf(filename, 64, "./res/vpss_fmt_out_ng_%d_2tap.bin", i);
			SAMPLE_COMM_FRAME_SaveToFile(filename, &stVideoFrame);
...
static CVI_S32 _MARS_VPSS_YUV_GOLDEN2K_TEST(bool fail_pause)
{
	SIZE_S stSize = {.u32Width = 1920, .u32Height = 1080};
	VPSS_GRP_ATTR_S    stVpssGrpAttr;
	VPSS_GRP	   VpssGrp = 0;
	...
	bool is_result_match = CVI_TRUE;
	const int fmt_in_idx = 4; // yuv420
	CVI_U8 testcase_vpss_chns[3] = { 1, 2, 3 };
	...
	CVI_VPSS_SetChnScaleCoefLevel(VpssGrp, VpssChn, VPSS_SCALE_COEF_BILINEAR);
	...
	for (VpssGrp = 0; VpssGrp < 3; ++VpssGrp) {
		...
		for (VpssChn = 0; VpssChn < testcase_vpss_chns[VpssGrp]; ++VpssChn) {
			...
			/* Only sc_v1 is 4tap */
			if (VpssChn == 1)
				snprintf(filename, 64, "./res/vpss_%d_%d.bin", VpssGrp, VpssChn);
			else
				snprintf(filename, 64, "./res/vpss_%d_%d_2tap.bin", VpssGrp, VpssChn);
			if (!SAMPLE_COMM_FRAME_CompareWithFile(filename, &stVideoFrame)) {
				CVI_TRACE_LOG(CVI_DBG_ERR, "grp%d chn%d. mismatch!\n", VpssGrp, VpssChn);
				is_result_match = CVI_FALSE;

				if (VpssChn == 1)
					snprintf(filename, 64, "./res/vpss_ng_%d_%d.bin", VpssGrp, VpssChn);
				else
					snprintf(filename, 64, "./res/vpss_ng_%d_%d_2tap.bin", VpssGrp, VpssChn);
				SAMPLE_COMM_FRAME_SaveToFile(filename, &stVideoFrame);
...
CVI_S32 _MARS_VPSS_YUV_GOLDEN4K_TEST(bool fail_pause)
{
	...
	for (VpssChn = 0; VpssChn < 3; ++VpssChn) {
		s32Ret = CVI_VPSS_GetChnFrame(VpssGrp, VpssChn, &stVideoFrame, 10000);
		if (s32Ret != CVI_SUCCESS) {
			CVI_TRACE_LOG(CVI_DBG_ERR, "CVI_VPSS_GetChnFrame for grp%d chn%d. s32Ret: 0x%x !\n"
				, VpssGrp, VpssChn, s32Ret);
			is_result_match = CVI_FALSE;
...
static CVI_S32 _MARS_VPSS_RGB_GOLDEN2K_TEST(bool fail_pause)
{
	SIZE_S stSize = {.u32Width = 1920, .u32Height = 1080};
	VPSS_GRP_ATTR_S    stVpssGrpAttr;
	VPSS_GRP	   VpssGrp = 4;
	...
	for (VpssChn = 0; VpssChn < 2; ++VpssChn) {
		...
		/* Only sc_v1 is 4tap */
		if (VpssChn == 1)
			snprintf(filename, 64, "./res/vpss_%d_%d.bin", 4, VpssChn);
		else
			snprintf(filename, 64, "./res/vpss_%d_%d_2tap.bin", 4, VpssChn);
		if (!SAMPLE_COMM_FRAME_CompareWithFile(filename, &stVideoFrame)) {
			CVI_TRACE_LOG(CVI_DBG_ERR, "grp%d chn%d. mismatch!\n", VpssGrp, VpssChn);
			is_result_match = CVI_FALSE;
...
static CVI_S32 _MARS_VPSS_YUV_GOLDEN2K_DUAL_TEST(bool fail_pause)
{
	SIZE_S stSize = {.u32Width = 1920, .u32Height = 1080};
	VPSS_GRP_ATTR_S    stVpssGrpAttr;
	VPSS_GRP	   VpssGrp = 0;
	...
	CVI_S32 s32Ret = CVI_SUCCESS;
	bool is_result_match = CVI_TRUE;
	CVI_U8 testcase_vpss_chns[2] = { 1, 2,};
	VPSS_CROP_INFO_S stCropInfo;
...
static CVI_S32 _MARS_VPSS_GRP_PARAM_TEST(bool fail_pause)
{
...
static CVI_S32 _MARS_VPSS_CHN_PARAM_TEST(bool fail_pause)
{
	VPSS_GRP_ATTR_S stGrpAttr;
...
static CVI_S32 _MARS_VPSS_YUV422_RGB_GOLDEN2K_DUAL_TEST(bool fail_pause)
{
	SIZE_S stSize = {.u32Width = 1920, .u32Height = 1080};
	VPSS_GRP_ATTR_S    stVpssGrpAttr;
	VPSS_GRP	   VpssGrp = 9;
...
static CVI_S32 _MARS_VPSS_GRP_ATTR_STRESS_TEST(void)
{
	VPSS_GRP VpssGrp = 0;
	VPSS_GRP_ATTR_S stVpssGrpAttr;
	VPSS_CHN VpssChn = VPSS_CHN0;
	...
	// 1. set max width/height (4000, 3000) to avoid exceeding ion size on 182x
	// 2. Currently only test input w/h > 32 case
	for (i = 0; i < 2; ++i) { // Use different chn attr settings
		CVI_U32 u32MaxGrpW = MIN(astVpssChnAttr[i].u32Width*32, 4000);
		CVI_U32 u32MaxGrpH = MIN(astVpssChnAttr[i].u32Height*32, 3000);
...
static CVI_S32 _MARS_VPSS_CHN_ATTR_STRESS_TEST(void)
{
	VPSS_GRP VpssGrp = 0;
	...
	for (i = 0; i < 2; ++i) { // Use different grp attr settings
		...
		w_gap = (4000 - u32MinChnW) / (u32TotalSteps - 1);
		h_gap = (3000 - u32MinChnH) / (u32TotalSteps - 1);
...
struct vpss_test_ops vpss_test_ops_mars = {
	.vi_test = _MARS_VPSS_VI_TEST,
	.in_fmt_test = _MARS_VPSS_FMT_INPUT_TEST,
	.out_fmt_test = _MARS_VPSS_FMT_OUTPUT_TEST,
	.yuv_2k_test = _MARS_VPSS_YUV_GOLDEN2K_TEST,
	.yuv_4k_test = NULL,
	.rgb_test = _MARS_VPSS_RGB_GOLDEN2K_TEST,
	.dual_yuv_test = _MARS_VPSS_YUV_GOLDEN2K_DUAL_TEST,
	.dual_combo_test = _MARS_VPSS_YUV422_RGB_GOLDEN2K_DUAL_TEST,
	.param_grp_test = _MARS_VPSS_GRP_PARAM_TEST,
	.param_chn_test = _MARS_VPSS_CHN_PARAM_TEST,
	.grp_attr_stress_test = _MARS_VPSS_GRP_ATTR_STRESS_TEST,
	.chn_attr_stress_test = _MARS_VPSS_CHN_ATTR_STRESS_TEST,
};
