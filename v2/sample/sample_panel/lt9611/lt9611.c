...
/***********************************************************/
#define single_port_mipi_A 0
#define single_port_mipi_B 1
...
struct Lontium_IC_Mode lt9611 = {
	single_port_mipi_A, // mipi_port_cnt; //single_port_mipi_A(Pin38~46) or single_port_mipi_B(Pin49~58) or dual_port_mipi
...
static optionExt long_option_ext[] = {
	{{"port", optional_argument, NULL, 'p'}, ARG_STRING, 0, 0, "select input port"},
	{{"resolution", optional_argument, NULL, 'r'}, ARG_STRING, 0, 0, "select hdmi display solution"},
	{{"help", no_argument, NULL, 'h'}, ARG_STRING, 0, 0, "print usage."},
...
static LT9611_VIDEO_FORMAT_E g_video_format = E_LT9611_VIDEO_2560x1440_60HZ;
...
static struct video_timing video_2560x1440_60Hz = {248, 44, 148, 2560, 3000, 3, 5, 33, 1440, 1481, 1, 1, 0, AR_16_9, 266580};
/*
...
static int lt9611_i2c_write_byte(CVI_U8 u8RegAddr, CVI_U8 u8Data)
{
	...
	if (ret < 0)
	{
		printf("lt9611_i2c_write_byte, addr: %x error!!!", u8RegAddr);
		return CVI_FAILURE;
	}
...
static int lt9611_i2c_read_byte(CVI_U8 u8RegAddr)
{
	...
	if (ret < 0)
	{
		printf("lt9611_i2c_read_byte, addr: %x error!!!", u8RegAddr);
		return -1;
	}
...
void LT9611_MIPI_Input_Analog(void) // xuxi
{
	...
	else if (lt9611.mipi_port_cnt == single_port_mipi_B)
	{
		lt9611_i2c_write_byte(0x14, 0x09);
	}
...
void LT9611_MIPI_Input_Digtal(void) // weiguo
{
	...
	printf("LT9611_MIPI_Input_Digtal: lt9611 set mipi lanes = %d", lanes);
	...
	if ((lt9611.mipi_port_cnt == single_port_mipi_A) || (lt9611.mipi_port_cnt == single_port_mipi_B)) //(P10) //single_port_mipi
	{
	...
	if (lt9611.mipi_mode == csi)
	{
		...
		printf("LT9611_MIPI_Input_Digtal: LT9611.mipi_mode = dsi\n");
#endif
	if (lt9611.mipi_port_cnt == single_port_mipi_B)
	{
		...
		lt9611_i2c_write_byte(0xff, 0x83);
		lt9611_i2c_write_byte(0x03, 0x40); // signal port from portB
	}
}
...
void LT9611_Video_Check(void) // dsren
{
	...
	{
		printf("LT9611_Video_Check: lt9611.input_color_space = YUV422\n");
		h_act_a /= 2;
		h_act_b /= 2;
	}
	else if (lt9611.input_color_space == RGB888)
	{
		printf("LT9611_Video_Check: lt9611.input_color_space = RGB888\n");
		h_act_a /= 3;
		h_act_b /= 3;
	}
	...
	printf("LT9611_Video_Check: h_act_a, h_act_b, v_act, v_tal: %d, %d, %d, %d, ", h_act_a, h_act_b, v_act, v_tal);
	printf("LT9611_Video_Check: mipi_video_format: 0x%x", mipi_video_format);
	...
	printf("LT9611_Video_Check: Video_Format =\n");
	///////////////////////formate detect///////////////////////////////////
...
void LT9611_PLL(struct video_timing *video_format) // zhangzhichun
{
	...
	printf("LT9611_PLL: set rx pll = %d", pclk);
	...
	printf("LT9611_PLL: pcr_m = 0x%x, hdmi_post_div = %d", pcr_m, hdmi_post_div); // Hex
	...
	for (i = 0; i < 6; i++)
	{
		...
		{
			printf("LT9611_PLL: HDMI pll locked band out: 0x%x", band_out);
			break;
		}
		else
		{
...
void LT9611_HDMI_TX_Digital(struct video_timing *video_format) // dsren
{
	...
	printf("LT9611_HDMI_TX_Digital: infoFrame_en = 0x%x", infoFrame_en);
...
void LT9611_Frequency_Meter_Byte_Clk(void)
{
	...
	lt9611_i2c_write_byte(0xff, 0x82);
	lt9611_i2c_write_byte(0xc7, 0x04);
	Timer0_Delay1ms(50);
	temp = lt9611_i2c_read_byte(0xcd);
	if ((temp & 0x60) == 0x60) /* clk stable */
	{
...
static void __upadte_video(LT9611_VIDEO_FORMAT_E enVideoFormat)
{
	...
	case E_LT9611_VIDEO_1920x1080_60HZ:
		...
		video = &video_1920x1080_60Hz;
		break;
	case E_LT9611_VIDEO_2560x1440_60HZ:
		Video_Format = video_2560x1440_60Hz_vic;
		video = &video_2560x1440_60Hz;
		break;
	default:
...
{
	SAMPLE_COMM_GPIO_SetValue(304, 1); // reset
	...
	while ((ch = getopt_long(argc, argv, "h", long_options, &idx)) != -1)
	{
		...
		case 'p':
			...
			{
				lt9611.mipi_port_cnt = single_port_mipi_A;
			}
