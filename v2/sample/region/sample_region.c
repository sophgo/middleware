...
CVI_S32 SAMPLE_REGION_VI_VPSS_VO_START(CVI_VOID)
{
	...
	s32Ret = SAMPLE_COMM_VPSS_Bind_VO(0, 0, 1, 0);
	if (s32Ret != CVI_SUCCESS) {
...
CVI_S32 SAMPLE_REGION_VI_VPSS_VO_2LAYER(CVI_VOID)
{
	...
	SAMPLE_INI_CFG_S	   stIniCfg = {0};
	SAMPLE_VI_CONFIG_S stViConfig;
	CVI_BOOL abChnEnable[VPSS_MAX_PHY_CHN_NUM] = {true};
	SAMPLE_VO_CONFIG_S stVoConfig;
	...
	s32Ret = SAMPLE_COMM_VPSS_Bind_VO(1, 0, 1, 0);
	if (s32Ret != CVI_SUCCESS) {
		SAMPLE_PRT("Bind vpss vo failed!\n");
		goto VPSS_BIND_VO_FAILED;
	}
...
VPSS_BIND_VO_FAILED:
	...
	SAMPLE_COMM_VI_DestroyVi(&stViConfig);
START_VO_FAILED:
	SAMPLE_COMM_VPSS_UnBind_VPSS(0, 0, 1);
	SAMPLE_COMM_VI_UnBind_VPSS(0, 0, 0);
VI_BIND_VPSS_FAILED:
...
CVI_S32 SAMPLE_REGION_VPSS_OSD_MULTICHN(CVI_VOID)
{
	...
	s32Ret = SAMPLE_COMM_VPSS_Bind_VO(3, 0, 1, 0);
	if (s32Ret != CVI_SUCCESS) {
		SAMPLE_PRT("Bind vpss vo failed!\n");
		goto VPSS_BIND_VO_FAILED;
	}
	...
	stRgnOverlay.unAttr.stOverlay.stCompressInfo.enOSDCompressMode = OSD_COMPRESS_MODE_NONE;
	...
	SAMPLE_COMM_VPSS_UnBind_VO(3, 0, 1, 0);
VPSS_BIND_VO_FAILED:
	stVoConfig.VoDev = 1;
	stVoConfig.enVoMode = VO_MODE_1MUX;
	SAMPLE_COMM_VO_StopVO(&stVoConfig);
START_VO_FAILED:
	SAMPLE_COMM_VPSS_UnBind_VPSS(2, 0, 3);
	SAMPLE_COMM_VPSS_UnBind_VPSS(1, 0, 2);
	SAMPLE_COMM_VPSS_UnBind_VPSS(0, 0, 1);
	SAMPLE_COMM_VI_UnBind_VPSS(0, 0, 0);
VPSS_BIND_VPSS_FAILED:
	SAMPLE_COMM_VPSS_Stop(0, abChnEnable);
