...
CVI_S32 SAMPLE_COMM_ISP_Run(CVI_U8 IspDev)
{
	...
	*arg = IspDev;
	...
	if (s32Ret == EPERM) {
		CVI_TRACE_LOG(CVI_DBG_WARN, "Failed to create RT thread (EPERM), retrying with normal priority...\r\n");
...
CVI_S32 SAMPLE_COMM_ISP_PatchSnsObj(CVI_U32 u32SnsId, SAMPLE_SENSOR_INFO_S *pstSnsInfo)
{
	...
	return (pstSnsObj->pfnPatchRxAttr) ? pstSnsObj->pfnPatchRxAttr(u32SnsId, &stRxInitAttr) : CVI_SUCCESS;
}
						...
						CVI_S32 s32I2cAddr)
{
	...
	if (pstSnsObj->pfnPatchI2cAddr)
		pstSnsObj->pfnPatchI2cAddr(u32SnsId, s32I2cAddr);
