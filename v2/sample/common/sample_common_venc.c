		...
		FILE *pFd, VENC_STREAM_S *pstStream)
{
	...
	for (CVI_U32 i = 0; i < pstStream->u32PackCount; i++) {
		...
		// printf("pack[%d], PTS = %"PRId64", DTS = %"PRId64", DataType = %d\n",
		// 		i, ppack->u64PTS, ppack->u64DTS, dataType);
		// printf("Addr = %p, Len = 0x%X, Offset = 0x%X\n",
...
static CVI_S32 SAMPLE_COMM_VENC_DetachVbPool(VENC_CHN VencChn)
{
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("CVI_VENC_GetChnAttr vechn[%d] failed with %#x!\n",
				VencChn, s32Ret);
		return CVI_FAILURE;
	}
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("CVI_VENC_GetModParam vechn[%d] failed with %#x!\n",
				VencChn, s32Ret);
		return CVI_FAILURE;
	}
	...
	if (eVbSource == VB_SOURCE_USER) {
		...
		if (s32Ret != CVI_SUCCESS) {
			printf("CVI_VENC_DetachVbPool vechn[%d] failed with %#x!\n",
					VencChn, s32Ret);
			return CVI_FAILURE;
		}
	}
...
CVI_S32 SAMPLE_COMM_VENC_Stop(VENC_CHN VencChn)
{
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("SAMPLE_COMM_VENC_DetachVbPool vechn[%d] failed with %#x!\n",
				VencChn, s32Ret);
		return CVI_FAILURE;
	}
...
CVI_S32 SAMPLE_COMM_VENC_GetGopAttr(VENC_GOP_MODE_E enGopMode, VENC_GOP_ATTR_S *pstGopAttr)
{
	...
	case VENC_GOPMODE_SMARTP:
		...
		printf("u32BgInterval %d, s32BgQpDelta %d, s32ViQpDelta %d\n",
	...
	default:
		printf("not support the gop mode %d!\n", enGopMode);
		return CVI_FAILURE;
	}
	return CVI_SUCCESS;
}
...
CVI_S32 SAMPLE_COMM_VENC_SetRoiAttr(VENC_CHN VencChn, PAYLOAD_TYPE_E enType)
{
	...
	if (enType == PT_H264) {
		...
		printf("SetRoiAttr only support H264/H265!\n");
		return CVI_FAILURE;
	}
	...
	*pbQpMapValid = CVI_FALSE;
	for (i = 0; i < MAX_NUM_ROI; ++i) {
		...
		if (s32Left > s32Right || s32Right >= ctbStride) {
			printf("s32Left = %d, s32Right = %d, ctbStride = %d\n",
		...
		if (s32Top > s32Bottom || s32Bottom >= ctbHeight) {
			printf("s32Top = %d, s32Bottom = %d, ctbHeight = %d\n",
					s32Top, s32Bottom, ctbHeight);
			return CVI_FAILURE;
		}

		printf("s32Left = %d, s32Right = %d, s32Top = %d, s32Bottom = %d\n",
	...
	printf("ctbStride = %d, ctbHeight = %d, QpMap = %p\n",
...
CVI_S32 SAMPLE_COMM_VENC_LoadRoiCfgFile(SAMPLE_COMM_VENC_ROI *vencRoi, CVI_CHAR *cfgFileName)
{
	...
	if (cfgFile == NULL) {
		printf("Missing ROI config file, %s\n", cfgFileName);
	...
	while (fgets(line, 256, cfgFile) != NULL) {
		...
		if (sscanf(line, "%u %d %d %d %d %u %u %u %u",
				...
				&_roi.u32FrameStart,
				&_roi.u32FrameEnd) == 0) {
			printf("Failed to parse roi cfg file, %s\n", cfgFileName);
			s32Ret = CVI_FAILURE_ILLEGAL_PARAM;
			break;
		}

		if (_roi.stVencRoiAttr.u32Index >= MAX_NUM_ROI) {
			printf("Incorrect index %d in roi cfg file, %s\n",
		...
		printf("roi frameStart %d ,frameEnd %d\n",
		...
		VENC_GOP_ATTR_S *pstGopAttr)
{
	...
	if (enType != PT_JPEG) {
		...
		if (s32Ret != CVI_SUCCESS) {
			printf("SAMPLE_COMM_VENC_SetRcParam, %d\n", s32Ret);
		...
		if (s32Ret != CVI_SUCCESS) {
			printf("SAMPLE_COMM_VENC_SetRefParam, %d\n", s32Ret);
			CVI_VENC_DestroyChn(VencChn);
			goto ERR_SAMPLE_COMM_VENC_CREATE;
		}
		...
		if (s32Ret != CVI_SUCCESS) {
			printf("SAMPLE_COMM_VENC_SetCuPrediction, %d\n", s32Ret);
			CVI_VENC_DestroyChn(VencChn);
			goto ERR_SAMPLE_COMM_VENC_CREATE;
		}
		...
		if (s32Ret != CVI_SUCCESS) {
			printf("SAMPLE_COMM_VENC_SetFrameLost, %d\n", s32Ret);
			CVI_VENC_DestroyChn(VencChn);
			goto ERR_SAMPLE_COMM_VENC_CREATE;
		}
		...
		if (s32Ret != CVI_SUCCESS) {
			printf("SAMPLE_COMM_VENC_SetSuperFrame, %d\n", s32Ret);
			CVI_VENC_DestroyChn(VencChn);
			goto ERR_SAMPLE_COMM_VENC_CREATE;
		}
	}
	...
	if (enType != PT_JPEG && enType != PT_MJPEG) {
		...
		if (eVbSource == VB_SOURCE_USER) {
			...
			printf("CVI_VENC_AttachVbPool chn[%d]\n",  VencChn);
			s32Ret = CVI_VENC_AttachVbPool(VencChn, &stPool);
			if (s32Ret != CVI_SUCCESS) {
				printf("CVI_VENC_AttachVbPool, %d\n", s32Ret);
				goto ERR_SAMPLE_COMM_VENC_CREATE;
			}
		}
	}
	...
	if (enType == PT_H265) {
		...
		if (s32Ret != CVI_SUCCESS) {
			printf("SAMPLE_COMM_VENC_SetH265Trans, %d\n", s32Ret);
			goto ERR_SAMPLE_COMM_VENC_CREATE;
		}

		s32Ret = SAMPLE_COMM_VENC_SetH265Vui(pIc, VencChn);
		if (s32Ret != CVI_SUCCESS) {
			printf("SAMPLE_COMM_VENC_SetH265Vui, %d\n", s32Ret);
			goto ERR_SAMPLE_COMM_VENC_CREATE;
		}

		s32Ret = SAMPLE_COMM_VENC_SetH265SliceSplit(pIc, VencChn);
		if (s32Ret != CVI_SUCCESS) {
			printf("SAMPLE_COMM_VENC_SetH265SliceSplit, %d\n", s32Ret);
			goto ERR_SAMPLE_COMM_VENC_CREATE;
		}

		s32Ret = SAMPLE_COMM_VENC_SetH265Dblk(pIc, VencChn);
		if (s32Ret != CVI_SUCCESS) {
			printf("SAMPLE_COMM_VENC_SetH265Dblk, %d\n", s32Ret);
			goto ERR_SAMPLE_COMM_VENC_CREATE;
		}

		s32Ret = SAMPLE_COMM_VENC_SetH265Sao(pIc, VencChn);
		if (s32Ret != CVI_SUCCESS) {
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("SAMPLE_COMM_VENC_SetChnParam, %d\n", s32Ret);
		CVI_VENC_DestroyChn(VencChn);
		goto ERR_SAMPLE_COMM_VENC_CREATE;
	}
		...
		CVI_BOOL bRcnRefShareBuf)
{
	...
	CVI_S32 s32Ret = CVI_SUCCESS;
	VENC_GOP_EX_ATTR_S *pstGopExAttr = &pstVencChnAttr->stGopExAttr;
	...
	pstVencChnAttr->stVencAttr.bByFrame = CVI_TRUE; // get stream mode is slice mode or
													// frame mode ?
	pstVencChnAttr->stVencAttr.enRotation = pIc->u32Rotation;
	pstVencChnAttr->stVencAttr.enMirrorDirextion = pIc->u32MirrorDirection;
	pstVencChnAttr->stVencAttr.enPixelFormat = vencMapPixelFormat(pIc->pixel_format);
	pstVencChnAttr->stVencAttr.u32CmdQueueDepth = pIc->u32CmdQueueDepth;
	pstVencChnAttr->stVencAttr.enEncMode = VENC_MODE_RECOMMEND;

	// gop ex param
	pstGopExAttr->u32GopPreset = pIc->u32GopPreset;
	...
	if (pstGopAttr->enGopMode == VENC_GOPMODE_NORMALP) {
		...
		printf("s32BgQpDelta = %d\n", pstGopAttr->stSmartP.s32BgQpDelta);
		printf("s32ViQpDelta = %d\n", pstGopAttr->stSmartP.s32ViQpDelta);
	} else {
		u32StatTime = pIc->statTime;
	}
	...
	case PT_H265: {
		if (enRcMode == SAMPLE_RC_CBR) {
			...
			printf("u32Gop = %d, u32IQp = %d, u32PQp = %d\n",
					pstH265FixQp->u32Gop,
					pstH265FixQp->u32IQp,
					pstH265FixQp->u32PQp);
			...
			printf("u32StatTime = %d, u32Gop = %d, u32MaxBitRate = %d\n",
	...
	case PT_H264: {
		if (enRcMode == SAMPLE_RC_CBR) {
			...
			printf("H.264 enRcMode(%d) not support\n", enRcMode);
			return CVI_FAILURE;
		}
	...
	case PT_MJPEG: {
		if (enRcMode == SAMPLE_RC_FIXQP) {
			...
			if (enRcMode == SAMPLE_RC_AVBR)
				printf("Mjpege not support AVBR, so change rcmode to VBR!\n");
	...
	VENC_CHN VencChn)
{
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("GetFrameLostStrategy failed!\n");
		return CVI_FAILURE;
	}
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("SetFrameLostStrategy failed!\n");
		return CVI_FAILURE;
	}
	...
	VENC_CHN VencChn)
{
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("GetSuperFrameStrategy failed!\n");
		return CVI_FAILURE;
	}
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("SetSuperFrameStrategy failed!\n");
		return CVI_FAILURE;
	}
...
CVI_S32 SAMPLE_COMM_VENC_SetModParam(const commonInputCfg *pCic)
{
	...
	case VB_SOURCE_COMMON:
		printf("vbMode VB_SOURCE_COMMON\n");
		break;
	case VB_SOURCE_MODULE:
		printf("vbMode VB_SOURCE_MODULE\n");
		break;
	case VB_SOURCE_PRIVATE:
		printf("vbMode VB_SOURCE_PRIVATE\n");
		break;
	case VB_SOURCE_USER:
		printf("vbMode VB_SOURCE_USER\n");
		break;
	default:
		printf("Invalid VB mode %d. Force to Priavte Mode.\n", eVbSource);
	...
	for (VENC_MODTYPE_E modtype = MODTYPE_H264E; modtype <= MODTYPE_JPEGE; modtype++) {
		...
		s32Ret = CVI_VENC_GetModParam(&stModParam);
		if (s32Ret != CVI_SUCCESS) {
			printf("CVI_VENC_GetModParam type %d failure\n", modtype);
			return CVI_FAILURE;
		}
		...
		case MODTYPE_JPEGE:
			...
			break;
		default:
			printf("SAMPLE_COMM_VENC_SetModParam invalid type %d failure\n", modtype);
			return CVI_FAILURE;
		}

		s32Ret = CVI_VENC_SetModParam(&stModParam);
		if (s32Ret != CVI_SUCCESS) {
			printf("CVI_VENC_SetModParam type %d failure\n", modtype);
			return CVI_FAILURE;
		}
	}
...
CVI_S32 SAMPLE_COMM_VENC_InitVBPool(vencChnCtx *pvecc, VENC_CHN VencChnIdx)
{
	...
	if (pvecc->stSize.u32Width || pvecc->stSize.u32Height) {
		...
		printf("Invalid width or height[%d][%d]\n",
	...
	u32BlkSize = COMMON_GetVencFrameBufferSize(pvecc->enPayLoad,
					...
					stSize.u32Height);
	if (u32BlkSize == 0) {
		printf("Invalid type for VENC to calculate frame buffer size\n");
		return CVI_FAILURE;
	}
	...
	printf("CVI_VB_CreatePool : id:%d, u32BlkSize=0x%x, u32BlkCnt=%d chn[%d]\n",
...
CVI_S32 SAMPLE_COMM_VENC_SetH264Trans(chnInputCfg *pIc, VENC_CHN VencChn)
{
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("CVI_VENC_GetH264Trans, %d\n", s32Ret);
		return s32Ret;
	}
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("CVI_VENC_SetH264Trans, %d\n", s32Ret);
		return s32Ret;
	}
...
CVI_S32 SAMPLE_COMM_VENC_SetH265Trans(chnInputCfg *pIc, VENC_CHN VencChn)
{
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("CVI_VENC_GetH265Trans, %d\n", s32Ret);
		return s32Ret;
	}
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("CVI_VENC_SetH265Trans, %d\n", s32Ret);
		return s32Ret;
	}
...
CVI_S32 SAMPLE_COMM_VENC_SetChnParam(chnInputCfg *pIc, VENC_CHN VencChn)
{
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("CVI_VENC_SetChnParam fail\n");
		return CVI_FAILURE;
	}
	...
	VENC_CHN VencChn)
{
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("GetH264SliceSplit failed!\n");
		return CVI_FAILURE;
	}
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("SetH264SliceSplit failed!\n");
		return CVI_FAILURE;
	}
	...
	VENC_CHN VencChn)
{
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("GetH265SliceSplit failed!\n");
		return CVI_FAILURE;
	}
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("SetH265SliceSplit failed!\n");
		return CVI_FAILURE;
	}
	...
	VENC_CHN VencChn)
{
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("GetH264IntraPred failed!\n");
		return CVI_FAILURE;
	}
...
CVI_S32 SAMPLE_COMM_VENC_SetH265Sao(chnInputCfg *pIc, VENC_CHN VencChn)
{
	VENC_H265_SAO_S h265Sao = { 0 };
	CVI_S32 s32Ret = CVI_FAILURE;

	s32Ret = CVI_VENC_GetH265Sao(VencChn, &h265Sao);
	if (s32Ret != CVI_SUCCESS) {
		printf("CVI_VENC_GetH265Sao, %d\n", s32Ret);
		return s32Ret;
	}

	h265Sao.slice_sao_luma_flag = pIc->u32SaoEnable;
	h265Sao.slice_sao_chroma_flag = pIc->u32SaoEnable;

	s32Ret = CVI_VENC_SetH265Sao(VencChn, &h265Sao);
	if (s32Ret != CVI_SUCCESS) {
		printf("CVI_VENC_SetH265Sao, %d\n", s32Ret);
		return s32Ret;
	}
		...
		VENC_GOP_ATTR_S *pstGopAttr)
{
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("SAMPLE_COMM_VENC_Create failed with %d\n", s32Ret);
		return CVI_FAILURE;
	}
	...
	if (!strcmp(pIc->codec, "264") ||  !strcmp(pIc->codec, "265")) {
		...
		if (s32Ret != CVI_SUCCESS) {
			printf("CVI_VENC_StartRecvPic failed get initial info with %d\n", s32Ret);
			return CVI_FAILURE;
		}
