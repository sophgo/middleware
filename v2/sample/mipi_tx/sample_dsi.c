...
int main(int argc, char *argv[])
{
	...
	if (argc == 1) {
		...
		devno = dev_cfg.devno = atoi(argv[1]);
	}

	if (devno >= 2) {
		SAMPLE_PRT("error devno in dev_cfg, it should be less than 2\n");
	}

	fd = open(devno == 0 ? MIPI_TX0_NAME : MIPI_TX1_NAME, O_RDWR | O_NONBLOCK, 0);
	if (fd == -1) {
		SAMPLE_PRT("Cannot open '%s': %d, %s\n", devno == 0 ? MIPI_TX0_NAME : MIPI_TX1_NAME,
