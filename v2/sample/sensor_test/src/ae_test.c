...
static CVI_U32 calcExpLineX100(CVI_U8 sID, CVI_S32 expTime)
{
	CVI_U32 expLine = 0;

	expLine = expTime / sensorInfo[sID].fExpLineTime * 100;
	AAA_LIMIT(expLine, sensorInfo[sID].u32LExpLineMin * 100, sensorInfo[sID].u32LExpLineMax * 100);
...
static void _print_ae_info(CVI_U8 sID)
{
	...
	if (!sensorInfo[sID].bWDRMode) {
		...
		info_log("sensor LEexpT: %u, LEexpL: %u AG: %u, DG: %u, IG: %u\n\n",
			stExpInfo.u32ExpTime, calcExpLineX100(sID, stExpInfo.u32ExpTime) / 100,
		...
		info_log("sensor LEexpT: %u, LEexpL: %u, SEexpT: %u, SEexpL: %u, AG: %u, DG: %u, IG: %u\n\n",
			stExpInfo.u32ExpTime, calcExpLineX100(sID, stExpInfo.u32ExpTime) / 100,
			stExpInfo.u32ShortExpTime, calcExpLineX100(sID, stExpInfo.u32ShortExpTime) / 100,
			stExpInfo.u32AGain, stExpInfo.u32DGain, stExpInfo.u32ISPDGain);
	}
}
...
#define WDR_TEST_RATIO    4
	...
	curExpLine[AE_LE] = calcExpLineX100(sID, stExpInfo.u32ExpTime);
	preExpLine[AE_LE] = curExpLine[AE_LE];
	curLuma[AE_LE] = leLuma;
	preLuma[AE_LE] = curLuma[AE_LE];

	if (sensorInfo[sID].bWDRMode) {
		curExpLine[AE_SE] = calcExpLineX100(sID, stExpInfo.u32ShortExpTime);
	...
	while (loop) {
		...
		curExpLine[AE_LE] = calcExpLineX100(sID, stExpInfo.u32ExpTime);
		// curExpLine[AE_LE] = calcExpLineX100(sID, tmpExpTime[AE_LE]);
		curLuma[AE_LE] = leLuma;
		...
		if (sensorInfo[sID].bWDRMode) {
			curExpLine[AE_SE] = calcExpLineX100(sID, stExpInfo.u32ShortExpTime);
			// curExpLine[AE_SE] = calcExpLineX100(sID, tmpExpTime[AE_SE]);
			curLuma[AE_SE] = seLuma;
		...
		info_log("LE, L: %d, T: %d, E X 100: %d, LR: %d, ER: %d\n",
			curLuma[AE_LE],
			stExpInfo.u32ExpTime,
			curExpLine[AE_LE],
			lumaRatio[AE_LE],
		...
		if (sensorInfo[sID].bWDRMode) {
			...
			info_log("SE, L: %d, T: %d, E X 100: %d, LR: %d, ER: %d\n",
				curLuma[AE_SE],
		...
		do {
			...
			curExpLine[fid] = calcExpLineX100(sID, tmpExpTime[fid]);
