...
static CVI_S32 SAMPLE_VDEC(sampleVdec *psvdec)
{
	...
	if (s32Ret == STAT_ERR_VDEC_INIT_ATTR) {
		printf("STAT_ERR_VDEC_INIT_ATTR\n");
		return s32Ret;
	...
	if (isResetChnTestMode(pic)) {
		...
		for (int idx = 0; idx < num_testcase; idx++) {
			...
			if (s32Ret < 0) {
				printf("checkInputConfig failure\n");
				return s32Ret;
			}
			...
			if (s32Ret == STAT_ERR_VDEC_COMMON_START) {
				...
				printf("SAMPLE_VDEC_START\n");
				goto END2;
			}

			SAMPLE_VDEC_STOP(psvdec);

			printf("case idx %d done(total %d)\n", (idx + 1), num_testcase);
		}
		...
		if (s32Ret == STAT_ERR_VDEC_COMMON_START) {
			printf("STAT_ERR_VDEC_COMMON_START\n");
			goto END3;
		} else if (s32Ret != CVI_SUCCESS) {
			printf("SAMPLE_VDEC_START\n");
			return s32Ret;
