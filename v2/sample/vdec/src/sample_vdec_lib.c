...
#define VB_HEIGHT		8192
#define VB_WIDTH		8192
...
CVI_S32 SAMPLE_VDEC_START(sampleVdec *psvdec)
{
	...
	for (i = 0; i < psvdec->u32VdecNumAllChns; i++) {
		...
		pvdtps->bDumpYUV = pvdcic->bDumpYUV;

		if (pic->u32BindMode == VDEC_BIND_DISABLE) {
			if (pvdcic->bDumpYUV == 0) {
				MD5_Init(&pvdtps->tMD5Ctx);
			}
		}
...
static CVI_S32 vdecInitVBPool(sampleVdec *psvdec)
{
	...
	for (i = 0; i < psvdec->u32VdecNumAllChns; i++) {
		...
		if (psvdattr->enType == PT_JPEG || psvdattr->enType == PT_MJPEG) {
			...
			printf("chnPoolGroup[%d] = %d\n", i, chnPoolGroup[i]);
		}
	}

	for (i = 0; i < stVbConf.u32MaxPoolCnt; i++) {
		printf("VDec Init Pool[%d], u32BlkSize = %d, u32BlkCnt = %d\n",
...
static CVI_S32 vdecInitVBPoolH26X(sampleVdec *psvdec)
{
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("SAMPLE_COMM_VDEC_InitVBPool fail\n");
	}
		...
		CVI_S32 s32MilliSec_out)
{
	...
	snprintf(pvtp->inFilePath,
			...
			SAMPLE_STREAM_PATH);
	snprintf(pvtp->outFilePath,
			sizeof(pvtp->outFilePath), "%s",
			SAMPLE_STREAM_PATH);
...
CVI_S32 SAMPLE_VDEC_INIT_VB(sampleVdec *psvdec)
{
	...
	for (i = 0; i < psvdec->u32VdecNumAllChns; i++) {
		...
		if (s32Ret != CVI_SUCCESS) {
			printf("vdecInitAttr, %#x!\n", s32Ret);
			return STAT_ERR_VDEC_INIT_ATTR;
		}
		printf("[ch%d]BufWidth %d, BufHeight %d, MaxFrame %d\n",
	...
	if (pic->u32BindMode != VDEC_BIND_DISABLE) {
		...
		for (i = 0; i < psvdec->u32VdecNumAllChns; i++) {
			...
			if (pic->u32BindMode == VDEC_BIND_VPSS_VO) {
				...
				if (s32Ret != CVI_SUCCESS) {
					printf("SAMPLE_COMM_VO_GetDefConfig fail\n");
				}

				s32Ret = SAMPLE_COMM_VO_StopVO(&stVoConfig);
				if (s32Ret != CVI_SUCCESS) {
					printf("SAMPLE_COMM_VO_StopVO fail\n");
				}
			}

			s32Ret = SAMPLE_COMM_VPSS_Stop(i, abChnEnable);
			if (s32Ret != CVI_SUCCESS) {
				printf("SAMPLE_COMM_VPSS_Stop fail\n");
			}
		}
	}
}
...
int checkInputConfig(vdecInputCfg *pic)
{
	...
	for (idx = 0; idx < pic->u32NumAllChns; idx++) {
		...
		if (len <= 0) {
			printf("len, input_path = %s\n", pvdcic->input_path);
			return -1;
		}

		len = strlen(pvdcic->output_path);
		if (len <= 0) {
			printf("len, output_path = %s\n", pvdcic->output_path);
			return -1;
		}
	}
