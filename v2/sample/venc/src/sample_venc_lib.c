...
extern pthread_t gs_VencSendTask[VENC_MAX_CHN_NUM];
extern pthread_t gs_VencGetTask[VENC_MAX_CHN_NUM];

extern PIXEL_FORMAT_E vencMapPixelFormat(CVI_S32 pixel_format);
...
static optionExt venc_long_option_ext[] = {
	...
	{{"goppreset", optional_argument, NULL, 0}, ARG_INT, 0, 9,
		"goppreset [0, 9], default = 6"},
	{{"sao", optional_argument, NULL, 0}, ARG_INT, 0, 9,
		"sao enable flag [0, 1], default = 1"},
	{{"rotate", optional_argument, NULL, 0}, ARG_INT, 0, 3,
		"rotation angel [0, 3], default = 0"},
	{{"mirr", optional_argument, NULL, 0}, ARG_INT, 0, 3,
...
static CVI_S32 _vpss_outwidth_align64(chnInputCfg *pIc, VPSS_CHN VpssChn)
{
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("CVI_VPSS_GetChnAttr fail, chn idx = %d\n", VpssChn);
		return s32Ret;
	}
	...
	if (stChnAttr.u32Width & 0x3F) {
		...
		printf("VPSS (chn:%d) out width (%u) align to %u\n",
		...
		if (s32Ret != CVI_SUCCESS) {
			printf("CVI_VPSS_SetChnAttr fail, chn idx = %d\n", VpssChn);
			return s32Ret;
		}
...
CVI_VOID SAMPLE_VENC_HandleSig(CVI_S32 signo)
{
...
CVI_S32 venc_main(int argc, char **argv)
{
	...
	if (pcic->testMode == JPEG_CONTI_ENCODE_MODE) {
		...
		for (int idx = 0; idx < num_testcase; idx++) {
			...
			printf("test case idx %d done, total %d\n", (idx + 1), num_testcase);
		}

		s32Ret = CVI_VENC_DestroyChn(0);
		if (s32Ret != CVI_SUCCESS) {
			printf("CVI_VENC_DestroyChn vechn[%d] failed with %#x!\n",
					0, s32Ret);
			return CVI_FAILURE;
		}
		...
		if (pcic->testMode == BIND_VI_VPSS_VENC) {
			...
			if (s32Ret != CVI_SUCCESS) {
				printf("_initVPSSSetting failure(%d)\n", s32Ret);
				return s32Ret;
			}
		}
	...
	while ((ch = getopt_long(argc, argv, "c:w:h:i:o:n:", long_options, &idx)) != -1) {
		...
		case 0:
			...
			if (!strcmp(long_options[idx].name, "getBsMode")) {
				...
				pIc->u32SaoEnable = arg.uval;
			} else if (!strcmp(long_options[idx].name, "rotate")) {
				pIc->u32Rotation = arg.uval;
			} else if (!strcmp(long_options[idx].name, "mirr")) {
				...
				pIc->s32DisableIDRCount = arg.uval;
			} else if (!strcmp(long_options[idx].name, "enableIDRCnt")) {
				pIc->s32EnableIDRCount = arg.uval;
...
static CVI_S32 _SAMPLE_VENC_FRM_testVpssVenc(sampleVenc *psv)
{
	...
	if (pcic->u32ViWidth < 320 || pcic->u32ViHeight < 256 ||
		...
		printf("u32ViWidth = %d, u32ViHeight = %d\n",
				pcic->u32ViWidth, pcic->u32ViHeight);
		printf("u32VpssWidth = %d, u32VpssHeight = %d\n",
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("SAMPLE_PLAT_SYS_INIT, s32Ret: 0x%x !\n", s32Ret);
		return s32Ret;
	}
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("_SAMPLE_VENC_SBM_initVpss, 0x%X\n", s32Ret);
		return s32Ret;
	}
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("_SAMPLE_VENC_SBM_initVenc, 0x%X\n", s32Ret);
		return s32Ret;
	}
	...
	for (idx = 0; idx < pIc->num_frames; idx++) {
		...
		if (s32Ret != CVI_SUCCESS) {
			printf("_SAMPLE_VENC_readToVpss , %d\n", s32Ret);
			break;
		}
		...
		if (s32Ret == CVI_ERR_VENC_FRC_NO_ENC) {
			...
			printf("_SAMPLE_VENC_SendFrame, %d\n", s32Ret);
			break;
		}
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("_SAMPLE_VENC_deInitVpss, 0x%X\n", s32Ret);
		return s32Ret;
	}

	s32Ret = _SAMPLE_VENC_SBM_deInitVenc(psv);
	if (s32Ret != CVI_SUCCESS) {
		printf("_SAMPLE_VENC_SBM_deInitVenc, 0x%X\n", s32Ret);
		return s32Ret;
	}
...
static CVI_S32 _SAMPLE_VENC_initViVpss(sampleVenc *psv)
{
	...
	if (pcic->u32ViWidth && pcic->u32ViHeight) {
		...
		if (pcic->bindmode == VENC_BIND_VI && pcic->numChn > 1) {
			printf("VI bind mode only support 1 channel\n");
			return CVI_FAILURE;
		}
		...
		// if (s32Ret != CVI_SUCCESS) {
		// 	printf("SAMPLE_COMM_VI_DefaultConfig failure(%d)\n", s32Ret);
		// 	return s32Ret;
		// }
		sleep(2);
...
CVI_S32 SAMPLE_VENC_START(sampleVenc *psv)
{
	...
	if (!strcmp(pIc->codec, "265") ||
		...
		for (CVI_S32 s32ChnIdx = 0; s32ChnIdx < pcic->numChn; s32ChnIdx++) {
			...
			if (s32Ret) {
				printf("[Chn %d]sample venc init failed\n", s32ChnIdx);
				return CVI_FAILURE;
			}
			...
			if (pcic->testMode == JPEG_CONTI_ENCODE_MODE) {
				...
				if (s32Ret != CVI_SUCCESS) {
					printf("CVI_VENC_SetChnAttr [%d] failed with %d\n",
							s32ChnIdx, s32Ret);
					return s32Ret;
				}
			}
		...
		if (pcic->ifInitVb) {
			...
			if (s32Ret != CVI_SUCCESS) {
				printf("fail to init sys and vb, 0x%x\n", s32Ret);
				return s32Ret;
			}
		}
...
#if 0
			if (pcic->ifInitVb) {
				printf("s32ChnIdx = %d, pstFrameInfo = 0x%p\n",
...
CVI_S32 checkInputCfg(chnInputCfg *pIc)
{
	if (!strcmp(pIc->codec, "264") || !strcmp(pIc->codec, "265")) {
		...
		if (pIc->rcMode == SAMPLE_RC_CBR ||
			...
			if (pIc->rcMode == SAMPLE_RC_CBR ||
				pIc->rcMode == SAMPLE_RC_UBR) {
				if (pIc->bitrate <= 0) {
					printf("CBR / UBR bitrate must be not less than 0");
					return -1;
				}
				printf("bitrate = %d\n", pIc->bitrate);
	...
	if (pIc->s32EnableIDRCount < pIc->s32DisableIDRCount) {
		printf("EnableIDRCount:%d < DisableIDRCount:%d\n", pIc->s32EnableIDRCount,  pIc->s32DisableIDRCount);
		return -1;
	}
...
//	SIZE_S inFrmSize;
	...
	if (pvecc->enPayLoad != PT_JPEG) {
		...
		if ((pvecc->enPayLoad != PT_JPEG) && (pvecc->enPayLoad != PT_MJPEG)) {
			if (pcic->vbMode == VB_SOURCE_USER) {
				//create vb pool
				printf("vbMode VB_SOURCE_USER_MODE\n");
				s32Ret =  SAMPLE_COMM_VENC_InitVBPool(pvecc, chnNum);
				if (s32Ret != CVI_SUCCESS)
					printf("init mod common vb fail\n");
	...
	if (s32Ret != CVI_SUCCESS) {
		printf("_SAMPLE_VENC_LoadCfgFile %#x!\n", s32Ret);
		return CVI_FAILURE;
	}
	...
	if (pIc->bind_mode == VENC_BIND_DISABLE && strlen(pIc->input_path) != 0) {
		...
		if ((pIc->num_frames > 0) && ((pvecc->num_frames > (CVI_U32)pIc->num_frames) || pvecc->bCircleSend == CVI_TRUE))
			pvecc->num_frames = pIc->num_frames;

		printf("chnNum = %d, num_frames = %d\n", chnNum, pvecc->num_frames);
...
static CVI_S32 initSysAndVb(sampleVenc *psv)
{
	...
	for (int i = 0; i < pcic->numChn; i++) {
		...
		u32BlkSize = VENC_GetPicBufferSize(
						...
						DATA_BITWIDTH_8,
						COMPRESS_MODE_NONE);
		pvecc->u32BlkSize = u32BlkSize;
#ifdef ARCH_CV183X
		printf("arch183x u32BlkSize[%d]align[%d]\n",
		...
		for (CVI_U32 j = 0; j < stVbConf.u32MaxPoolCnt; j++) {
			if (u32BlkSize == stVbConf.astCommPool[j].u32BlkSize) {
				stVbConf.astCommPool[j].u32BlkCnt += pIc->u32MinSrcCount;
				bRepeated = CVI_TRUE;
				break;
			}
		}
		...
		if (bRepeated == CVI_FALSE) {
			...
			stVbConf.astCommPool[stVbConf.u32MaxPoolCnt].u32BlkCnt  = pIc->u32MinSrcCount;
			stVbConf.u32MaxPoolCnt++;
		}

		if (stVbConf.u32MaxPoolCnt > VB_MAX_COMM_POOLS) {
			printf("u32MaxPoolCnt %d > , VB_MAX_COMM_POOLS %d\n",
	...
	for (int i = 0; i < pcic->numChn; i++) {
		...
		pIc = &pvecc->chnIc;
		for (int j = 0; j < pIc->u32MinSrcCount; j++) {
			blk = CVI_VB_GetBlock(VB_INVALID_POOLID, pvecc->u32BlkSize);
			if (blk != VB_INVALID_HANDLE) {
				pvecc->frameUnusedQueue[j].u32PoolId = CVI_VB_Handle2PoolId(blk);
				pvecc->frameUnusedQueue[j].u64PhyAddr = CVI_VB_Handle2PhysAddr(blk);
				pvecc->frameUnusedQueue[j].iUseFlag = 0;
				printf("alloc frame chn:%d frame:%d,0x%lx size:%d\n"
					, i, j, pvecc->frameUnusedQueue[j].u64PhyAddr, pvecc->u32BlkSize);
			}
		}
	}
...
static CVI_S32 _SAMPLE_VENC_LoadCfgFile(vencChnCtx *pvecc)
{
	...
	if ((pvecc->enPayLoad == PT_H265) && strlen(pIc->qpMapCfgFile) > 0) {
		...
		if (s32Ret != CVI_SUCCESS) {
			printf("Venc LoadRoiCfgFile failed for %#x!\n", s32Ret);
			return CVI_FAILURE;
		}
...
static CVI_S32 SAMPLE_VENC_StartGetStream(vencChnCtx *pvecc, CVI_S32 s32ChnIdx)
{
	...
	printf("SAMPLE_VENC_StartGetStream\n");

	gs_VencSendTask[s32ChnIdx] = 0;
		...
		VIDEO_FRAME_INFO_S *pstVideoFrame)
{
	...
	if (strlen(pIc->input_path) != 0) {
		s32Ret = cviReadSrcFrame(pvecc->pstVFrame, pvecc->fpSrc, pvecc->bCircleSend);
		if (s32Ret != CVI_SUCCESS) {
			printf("(chn %d) cviReadSrcFrame fail\n", VencChn);
			return s32Ret;
		}
...
static CVI_BOOL _check_if_free_yuv(vencChnCtx *pvecc)
{
	CVI_BOOL has_free_yuv = CVI_FALSE;
	CVI_S32 index = 0;

	for (index = 0; index < (int)(pvecc->chnIc.u32MinSrcCount); index++) {
		if (pvecc->frameUnusedQueue[index].iUseFlag == 0) {
			has_free_yuv = 1;
			break;
		}
	}
...
static CVI_VOID *SAMPLE_VENC_SendVencFrameProc(CVI_VOID *pArgs)
{
	...
	CVI_S32 s32Ret = 0;
	CVI_S32 s32SendRet = 0, s32GetRet = 0;
	CVI_U32 i = 0;
	SIZE_S inFrmSize;
	CVI_S32 jpeg_perf_test = ((pvecc->enPayLoad == PT_JPEG || pvecc->enPayLoad == PT_MJPEG) && pvecc->perf == 1);
	...
	while (pvecc->chnStat == CHN_STAT_START) {
		...
		// test enable/disable IDR feature
		if (pvecc->enPayLoad == PT_H264 || pvecc->enPayLoad == PT_H265) {
			if (pIc->s32DisableIDRCount == (CVI_S32)i) {
				CVI_VENC_EnableIDR(VencChn, CVI_FALSE);
			}

			if (pIc->s32EnableIDRCount == (CVI_S32)i) {
		...
		if (i > pvecc->num_frames) {
			pvecc->pstFrameInfo->stVFrame.bSrcEnd = 1;
			pvecc->pstFrameInfo->stVFrame.s32FrameIdx = -1;
		}

RETRY_SEND_FRAME:
		s32SendRet = _SAMPLE_VENC_SendFrame(pvecc, i);
...
RETRY_GET_STREAM:
		...
		} else if (s32SendRet != CVI_SUCCESS) {
			printf("_SAMPLE_VENC_SendFrame, %d\n", s32SendRet);
			break;
		}

		if (pIc->bind_mode == VENC_BIND_DISABLE) {
			if (!jpeg_perf_test && pvecc->pstFrameInfo) {
				free_frame(pvecc->pstFrameInfo);
		...
		if (pvecc->enPayLoad == PT_JPEG || pvecc->enPayLoad == PT_MJPEG) {
			if (s32GetRet != CVI_SUCCESS) {
				printf("_SAMPLE_VENC_GetStream jpeg, %d\n", s32GetRet);
				break;
			}
		} else {
		...
		// // flush remain encoded frame
		// if ((pvecc->enPayLoad == PT_H264 || pvecc->enPayLoad == PT_H265) && (i == pvecc->num_frames)) {
		// 	printf("get remain encoded frame chn:%d\n", pvecc->VencChn);
		// 	do {
		// 		s32GetRet = _SAMPLE_VENC_GetStream(pvecc);
		// 		printf("get remain encoded frame chn:%d, ret:%d\n", pvecc->VencChn, s32GetRet);
		// 	} while (s32GetRet != CVI_ERR_VENC_GET_STREAM_END);
...
static CVI_S32 _SAMPLE_VENC_SendFrame(vencChnCtx *pvecc, CVI_U32 i)
{
	...
	if (enableQpMap) {
		...
		if (s32Ret != CVI_SUCCESS) {
			printf("SAMPLE_COMM_VENC_SetQpMapByCfgFile, %d\n", s32Ret);
			return s32Ret;
		}
		...
		if (s32Ret != CVI_SUCCESS) {
			printf("SAMPLE_COMM_VENC_SetRoiAttrByCfgFile, %d\n", s32Ret);
			return s32Ret;
		}
	}
...
static CVI_S32 _SAMPLE_VENC_GetStream(vencChnCtx *pvecc)
{
	...
	if (s32Ret != CVI_SUCCESS) {
		// printf("SAMPLE_COMM_VENC_SaveChannelStream, VencChn = %d, s32Ret = %d\n",
		// 		VencChn, s32Ret);
		return s32Ret;
	}
	...
	if (pIc->bind_mode == VENC_BIND_DISABLE) {
		...
		if (s32Ret != CVI_SUCCESS) {
			printf("_releaseNonBindModeSrcFrame fail\n");
			return s32Ret;
		}
	}
...
RETRY_GET_FRAME:
	if (pstVFrame->u32Width == pstVFrame->u32Stride[0]) {
		...
		for (j = 0; j < u32CbCrReadSrcHeight; j++) {
			...
			to_read = (pstVFrame->u32Width%2)? ((pstVFrame->u32Width+1)>> bCbWidthShift): ((pstVFrame->u32Width)>> bCbWidthShift);
			read_byte = fread((void *)frm_ptr, 1, to_read, fp);
			if (read_byte != to_read) {
...
static VIDEO_FRAME_INFO_S *allocate_frame(SIZE_S stSize, PIXEL_FORMAT_E enPixelFormat, vencChnCtx *pvecc)
{
	VIDEO_FRAME_INFO_S *pstVideoFrame = NULL;
	VIDEO_FRAME_S *pstVFrame;
	// VB_BLK blk;
	VB_CAL_CONFIG_S stVbCfg;
	CVI_BOOL bFindUnusedBuffer = 0;
	int i = 0;
	CVI_U32 u32PoolId = 0;
	CVI_U64 u64PhyAddr = 0;

	for (i = 0; i < (int)(pvecc->chnIc.u32MinSrcCount); i++) {
		if (pvecc->frameUnusedQueue[i].iUseFlag == 0) {
			pvecc->frameUnusedQueue[i].iUseFlag = 1;
			bFindUnusedBuffer = 1;
			u32PoolId = pvecc->frameUnusedQueue[i].u32PoolId;
			u64PhyAddr = pvecc->frameUnusedQueue[i].u64PhyAddr;
			break;
		}
	}

	if (bFindUnusedBuffer == 0) {
		return NULL;
	}

	pstVideoFrame = (VIDEO_FRAME_INFO_S *)calloc(sizeof(VIDEO_FRAME_INFO_S), 1);
	...
	if (pstVFrame->u32Width % VENC_ALIGN_W) {
		printf("u32Width is not algined to %d\n", VENC_ALIGN_W);
	}

	pstVFrame->s32FrameIdx = i;
	printf("send chn:%d srcidx:%d\n", pvecc->VencChn, pstVFrame->s32FrameIdx);
	pstVideoFrame->u32PoolId = u32PoolId;//CVI_VB_Handle2PoolId(blk);
	pstVFrame->u64PhyAddr[0] = u64PhyAddr;//CVI_VB_Handle2PhysAddr(blk);
