...
int os08b10_i2c_init(VI_PIPE ViPipe)
{
	...
	u8DevNum = g_aunOs08b10_BusInfo[ViPipe].s8I2cDev;
	...
	ret = ioctl(g_fd[ViPipe], I2C_SLAVE_FORCE, g_aunOs08b10_AddrInfo[ViPipe].s8I2cAddr);
	if (ret < 0) {
...
int os08b10_read_register(VI_PIPE ViPipe, int addr)
{
	...
	ret = write(g_fd[ViPipe], buf, os08b10_addr_byte);
	if (ret < 0) {
	...
	buf[0] = 0;
	buf[1] = 0;
	ret = read(g_fd[ViPipe], buf, os08b10_data_byte);
	if (ret < 0) {
		CVI_TRACE_SNS(CVI_DBG_ERR, "I2C_READ error!\n");
		return 0;
	}
...
int os08b10_write_register(VI_PIPE ViPipe, int addr, int data)
{
	...
	if (os08b10_data_byte == 1) {
		buf[idx] = data & 0xff;
		idx++;
	}

	ret = write(g_fd[ViPipe], buf, os08b10_addr_byte + os08b10_data_byte);
	if (ret < 0) {
