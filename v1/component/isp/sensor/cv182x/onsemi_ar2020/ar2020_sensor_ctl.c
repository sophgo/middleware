...
int ar2020_i2c_init(VI_PIPE ViPipe)
{
	...
	u8DevNum = g_aunAR2020_BusInfo[ViPipe].s8I2cDev;
	...
	ret = ioctl(g_fd[ViPipe], I2C_SLAVE_FORCE,  g_aunAr2020_AddrInfo[ViPipe].s8I2cAddr);
	if (ret < 0) {
...
int ar2020_read_register(VI_PIPE ViPipe, int addr)
{
	...
	if (ar2020_addr_byte == 2)
		buf[idx++] = (addr >> 8) & 0xff;

	// add address byte 0
	buf[idx++] = addr & 0xff;

	ret = write(g_fd[ViPipe], buf, ar2020_addr_byte);
	if (ret < 0) {
	...
	buf[0] = 0;
	buf[1] = 0;
	ret = read(g_fd[ViPipe], buf, ar2020_data_byte);
	if (ret < 0) {
...
int ar2020_write_register(VI_PIPE ViPipe, int addr, int data)
{
	...
	ret = write(g_fd[ViPipe], buf, ar2020_addr_byte + ar2020_data_byte);
	if (ret < 0) {
		CVI_TRACE_SNS(CVI_DBG_ERR, "I2C_WRITE error!\n");
		return CVI_FAILURE;
	}
