...
int imx585_i2c_init(VI_PIPE ViPipe)
{
	...
	u8DevNum = g_aunImx585_BusInfo[ViPipe].s8I2cDev;
	...
	ret = ioctl(g_fd[ViPipe], I2C_SLAVE_FORCE, g_aunImx585_AddrInfo[ViPipe].s8I2cAddr);
	if (ret < 0) {
...
int imx585_write_register(VI_PIPE ViPipe, int addr, int data)
{
	...
	if (imx585_data_byte == 1) {
		buf[idx] = data & 0xff;
		idx++;
	}

	ret = write(g_fd[ViPipe], buf, imx585_addr_byte + imx585_data_byte);
	if (ret < 0) {
