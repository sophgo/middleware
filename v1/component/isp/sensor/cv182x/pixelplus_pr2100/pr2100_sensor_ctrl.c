...
int pr2100_i2c_init(VI_PIPE ViPipe, CVI_U8 i2c_addr)
{
	...
	CVI_TRACE_SNS(CVI_DBG_ERR, "open %s\n", acDevFile);
...
int pr2100_read_register(VI_PIPE ViPipe, int addr)
{
	...
	CVI_TRACE_SNS(CVI_DBG_ERR, "i2c r 0x%x = 0x%x\n", addr, data);
	return data;
}
...
int pr2100_write_register(VI_PIPE ViPipe, int addr, int data)
{
	...
	// CVI_TRACE_SNS(CVI_DBG_ERR, "i2c w 0x%x 0x%x\n", addr, data);

#if 0 // read back checing
	ret = pr2100_read_register(ViPipe, addr);
	if (ret != data)
		CVI_TRACE_SNS(CVI_DBG_ERR, "i2c readback-check fail, 0x%x != 0x%x\n", ret, data);
#endif
	return CVI_SUCCESS;
}
...
void pr2100_init(VI_PIPE ViPipe)
{
	...
	if (pr2100_i2c_init(ViPipe, g_aunPr2100_MasterAddrInfo[ViPipe].s8I2cAddr) != CVI_SUCCESS) {
	...
	case PR2100_MODE_1080P25_4CH:
		...
		if (pr2100_i2c_init(slave_pipe, g_aunPr2100_SlaveAddrInfo[ViPipe].s8I2cAddr) != CVI_SUCCESS) {
...
static void pr2100_set_1080p25_4ch(VI_PIPE ViPipe)
{
	...
	pr2100_set_1080p25_4ch_slave(slave_pipe);
}
...
static void pr2100_set_1080p30_4ch(VI_PIPE ViPipe)
{
	...
	pr2100_set_1080p30_4ch_slave(slave_pipe);
}
