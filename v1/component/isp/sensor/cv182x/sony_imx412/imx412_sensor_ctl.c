...
int imx412_i2c_init(VI_PIPE ViPipe)
{
	...
	u8DevNum = g_aunImx412_BusInfo[ViPipe].s8I2cDev;
	...
	ret = ioctl(g_fd[ViPipe], I2C_SLAVE_FORCE, g_aunImx412_AddrInfo[ViPipe].s8I2cAddr);
	if (ret < 0) {
...
int imx412_write_register(VI_PIPE ViPipe, int addr, int data)
{
	...
	if (imx412_data_byte == 1) {
		buf[idx] = data & 0xff;
		idx++;
	}

	ret = write(g_fd[ViPipe], buf, imx412_addr_byte + imx412_data_byte);
	if (ret < 0) {
...
void imx412_mirror_flip(VI_PIPE ViPipe, ISP_SNS_MIRRORFLIP_TYPE_E eSnsMirrorFlip)
{
	...
	imx412_write_register(ViPipe, 0x3020, u8Mirror);
	imx412_write_register(ViPipe, 0x3021, u8Filp);
}
