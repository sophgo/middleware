...
int os05a20_i2c_init(VI_PIPE ViPipe)
{
	...
	u8DevNum = g_aunOs05a20_BusInfo[ViPipe].s8I2cDev;
	...
	ret = ioctl(g_fd[ViPipe], I2C_SLAVE_FORCE, g_aunOs05a20_AddrInfo[ViPipe].s8I2cAddr);
	if (ret < 0) {
...
int os05a20_read_register(VI_PIPE ViPipe, int addr)
{
	...
	ret = write(g_fd[ViPipe], buf, os05a20_addr_byte);
	if (ret < 0) {
	...
	buf[0] = 0;
	buf[1] = 0;
	ret = read(g_fd[ViPipe], buf, os05a20_data_byte);
	if (ret < 0) {
