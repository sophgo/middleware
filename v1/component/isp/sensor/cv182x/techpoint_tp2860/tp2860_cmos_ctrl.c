...
int tp2860_i2c_init(VI_PIPE ViPipe)
{
	...
	u8DevNum = g_auntp2860_BusInfo[ViPipe].s8I2cDev;
...
int tp2860_read_register(VI_PIPE ViPipe, int addr)
{
	...
	if (tp2860_addr_byte == 2)
		buf[idx++] = (addr >> 8) & 0xff;

	// add address byte 0
	buf[idx++] = addr & 0xff;

	ret = write(g_fd[ViPipe], buf, tp2860_addr_byte);
	if (ret < 0) {
	...
	buf[0] = 0;
	buf[1] = 0;
	ret = read(g_fd[ViPipe], buf, tp2860_data_byte);
	if (ret < 0) {
		CVI_TRACE_SNS(CVI_DBG_ERR, "I2C_READ error!\n");
		return 0;
	}
...
int tp2860_write_register(VI_PIPE ViPipe, int addr, int data)
{
	...
	ret = write(g_fd[ViPipe], buf, tp2860_addr_byte + tp2860_data_byte);
	if (ret < 0) {
