...
int ov2736_i2c_init(VI_PIPE ViPipe)
{
	...
	u8DevNum = g_aunOv2736_BusInfo[ViPipe].s8I2cDev;
	...
	ret = ioctl(g_fd[ViPipe], I2C_SLAVE_FORCE, ov2736_i2c_addr);
	if (ret < 0) {
...
int ov2736_read_register(VI_PIPE ViPipe, int addr)
{
	...
	if (ov2736_addr_byte == 2)
		buf[idx++] = (addr >> 8) & 0xff;

	// add address byte 0
	buf[idx++] = addr & 0xff;

	ret = write(g_fd[ViPipe], buf, ov2736_addr_byte);
	if (ret < 0) {
	...
	buf[0] = 0;
	buf[1] = 0;
	ret = read(g_fd[ViPipe], buf, ov2736_data_byte);
	if (ret < 0) {
...
int ov2736_write_register(VI_PIPE ViPipe, int addr, int data)
{
	...
	if (ov2736_data_byte == 1) {
		buf[idx] = data & 0xff;
		idx++;
	}

	ret = write(g_fd[ViPipe], buf, ov2736_addr_byte + ov2736_data_byte);
	if (ret < 0) {
		CVI_TRACE_SNS(CVI_DBG_ERR, "I2C_WRITE error!\n");
		return CVI_FAILURE;
	}
